...
| `narrowing.py` | 771 | Verify | The ONE derivation of whether a value narrows into a `@Nat` slot, read by BOTH the verifier's `guarded` claim and codegen's guard emission so the two cannot drift (#1362); the type oracle is a parameter because the verifier reads the checker's semantic types while codegen reads declared names.  Also THE classifier of an integer value's sign every widening guard and obligation reads (#1503): `result_is_nat()` (a literal-only value by its folded value, `literal_range()`), and for a destructured or pattern-bound component `component_sources()` / `component_is_nat()` / `component_narrows()`, which read the constructor argument where the source builds the component and the declaration only where it is opaque — never the checker's bottom-up type, which calls `0 - 3` a `@Nat`; `literal_operation_width()` gives an operation of two literal-only operands its width by value.  Also the ONE roster of the construction and bind sites whose §2.6.5 refinement predicate is runtime-guarded (`REFINED_BIND_GUARDED_SITES`, #1426), CONSULTED by codegen's guard emission rather than mirrored by it, so a site is added once and the guard and the guarded status move together | `is_static_nat_typed()`, `has_underflow_leaf()`, `narrows_into_nat()`, `result_is_nat()`, `component_sources()`, `component_is_nat()`, `literal_range()`, `REFINED_BIND_GUARDED_SITES` |
