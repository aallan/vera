...
| `module_view.py` | 115 | Resolve / Type check / Compile | The ONE derivation of what a namespace sees through its imports (#1489, #1493): the resolved modules a namespace reaches with `direct` re-scoped to it, and the data types it imports with every supplier, each read through `merged_import_filters()` — read by the checker's module registration and body checks and by code generation's per-module registrar and ADT membership, so a module's signatures and bodies are measured in the same namespace whichever file is the entry | `modules_visible_to()`, `imported_data_types()` |
