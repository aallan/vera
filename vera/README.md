...
| `prelude.py` | 1,243 | Type check | Standard prelude — built-in ADT and combinator injection; a combinator the program overrides stays in the program under its own identity, `mod$<prelude>$<name>`, and the prelude's bodies — and a module that does not declare the name — call that one (#1495) | `inject_prelude()`, `prelude_symbol()`, `entry_held_bare_names()`, `prelude_adt_names()`, `overridable_builtin_names()` |
...
| `monomorphize.py` | 4,368 | Resolve | Shared generic instantiation discovery + AST substitution (verifier and codegen); each clone's De Bruijn recount renders its binder names under the **origin module's** `AliasEnv`, the one its consumers rebuild the clone's scope with (#1208).  Also the ONE function-ownership rule both sides key symbols by (#1498): which module declaration owns the entry's bare name (`FnOwnership`), and the calls a module body binds to a qualified-only declaration (`module_call_renames`) | `substitute_type_vars()`, `resolve_type_alias()`, `canonicalize_type_aliases()`, `fn_ownership()`, `module_call_renames()` |
