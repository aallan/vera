...
|-----------|-----|---------|
| **Verification gaps that downgrade silently** | the effect-operation argument and the generic-instantiated constructor field have no codegen runtime guard, so an unverified compile can store a negative `@Nat` at one of those sites — every narrowing shape is still statically obligated (#552, #747), so *verified* programs are sound; generic (`forall<T>`) bodies skip all static verification; statement-position calls are never precondition-checked | [#754](https://github.com/aallan/vera/issues/754), [#757](https://github.com/aallan/vera/issues/757), [#555](https://github.com/aallan/vera/issues/555), [#730](https://github.com/aallan/vera/issues/730) |
