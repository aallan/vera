...
| **Refinement runtime guards: primitive bases only** | primitive-base refinements (`@Int`/`@Nat`/`@Bool`/`@Float64`/`@String`) are verified statically (#746) **and** runtime-guarded at the function boundary (a refined param/return predicate check traps an unverified compile's violating value); a refinement over a non-primitive base (e.g. `Array`) stays Tier-3 statically and is not refinement-runtime-guarded — the underlying operations' checks (e.g. array bounds) are the safety net | [#762](https://github.com/aallan/vera/issues/762) |
