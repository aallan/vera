...
| `checker/` | 9,308 | Type check | Two-pass type checker (mixin package) | `typecheck()` |
...
| `tail_position.py` | 106 | Verify / Compile | The ONE tail-position analysis (#517): which `FnCall` nodes of a body are in tail position.  Codegen lowers them to `return_call`, and the verifier reads the same answer to obligate a `decreases` measure on a self-recursive tail call's arguments, where the compiled code evaluates it before the call (#1480) | `compute_tail_call_sites()` |
...
| `  session.py` | 439 | | Warm-Z3 daemon: per-function replay vs re-verify in declaration order; clears the disclosed set per program and re-enters `_verify_source_fixpoint` for one program's fixpoint (#1363) | |
