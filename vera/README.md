...
| `trap_registry.py` | 471 | Compile / Run | The ONE statement of every runtime trap (#1479): the kinds a host can report (the code the `vera.trap` signal carries, the description, the Fix paragraph), every code-generation function that emits a check able to trap together with the verifier obligation kinds it is the runtime half of, and the `EmittedCheck` record `CompileResult.emitted_checks` lists per module | `TRAP_KINDS`, `TRAP_EMITTERS`, `EmittedCheck` |
