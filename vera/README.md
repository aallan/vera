...
|-----------|-----|---------|
| **Verification gaps that downgrade silently** | the effect-operation-argument `@Nat` narrowing has no codegen runtime guard, so an unverified compile can store a negative `@Nat` there (every narrowing shape is statically obligated — #552, #747); generic (`forall<T>`) bodies skip all static verification; statement-position calls are never precondition-checked | [#754](https://github.com/aallan/vera/issues/754), [#555](https://github.com/aallan/vera/issues/555), [#730](https://github.com/aallan/vera/issues/730) |
