...
| `prelude.py` | 1,243 | Type check | Standard prelude — built-in ADT and combinator injection; a combinator the program overrides stays in the program under its own identity, `<prelude>::<name>`, and the prelude's bodies — and a module that does not declare the name — call that one (#1495) | `inject_prelude()`, `prelude_symbol()`, `entry_overridden_prelude_fns()`, `prelude_adt_names()`, `overridable_builtin_names()` |
...
| `symbols.py` | 277 | Compile | The ONE builder and reader of every composed internal symbol (#1494): a module's `a.b::name`, the prelude's `<prelude>::name`, a hoisted helper's `$where$` step, a clone's `$<types>` step, and the runtime's `rt.` and the host's `vera.` names.  Each is spelled with a marker no identifier, path segment or escaped type can spell, so the encoding is injective by construction and `decode` recovers every part; `display` renders any symbol as the spelling a person wrote | `module_symbol()`, `prelude_symbol()`, `helper_symbol()`, `clone_symbol()`, `decode()`, `display()` |
| `call_targets.py` | 60 | Type check | Which declaration each call names, as the checker resolved it (#1494): per module, every call's target keyed by span, and for the entry, which module owns each bare name it reaches through an import.  Code generation and the verifier's discovery bind to this record rather than deriving a second answer | `CallTarget`, `CallResolution` |
...
| `monomorphize.py` | 4,368 | Resolve | Shared generic instantiation discovery + AST substitution (verifier and codegen); each clone's De Bruijn recount renders its binder names under the **origin module's** `AliasEnv`, the one its consumers rebuild the clone's scope with (#1208).  Also the ONE function-ownership rule both sides key symbols by (#1498), read off the checker's call resolution (#1494): which module declaration owns the entry's bare name (`FnOwnership`), and the binding of every module-body call to the declaration the checker resolved it to (`bind_module_calls`) | `substitute_type_vars()`, `resolve_type_alias()`, `canonicalize_type_aliases()`, `fn_ownership()`, `bind_module_calls()` |
...
| `  modules.py` | 2,172 | | Cross-module registration + call detection (C7e), per-module alias + source scopes (#1111/#1186) — `_module_alias_scope` swaps the alias maps *and* the `AliasEnv` every codegen rendering goes through as one pair (#1208) — the #1317 per-owner ADT rename (`_contended_adt_renames`), which decides which contended `data` declarations are qualified to `<path>::<Name>` and what each namespace calls them afterwards, and its function twin (#1498): every module function that does not own the entry's bare name is emitted as `<path>::<name>` and each module body's calls are bound to the declaration its own namespace resolves, so E608 fires only for the two-owner ambiguity | |
...
| `  functions.py` | 1,493 | | Function body compilation, GC prologue/epilogue (Pass 2) | |
...
`wasm/markdown.py` provides bidirectional WASM memory marshalling for the `MdInline` and `MdBlock` ADT trees. Write direction (`write_md_inline`, `write_md_block`) allocates ADT nodes in WASM linear memory using the same `$rt.alloc` + tag-dispatch layout as user-defined ADTs. Read direction (`read_md_inline`, `read_md_block`) reconstructs Python objects from WASM memory. Helper functions `_read_i32`, `_read_i64`, and `_write_i64` handle raw memory access for struct fields.
