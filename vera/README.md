...
| `  registration.py` | 1,199 | | Pass 1 forward declarations, ability registration | |
...
| `module_view.py` | 147 | Resolve / Type check / Compile | The ONE derivation of what a namespace sees through its imports (#1489, #1493): the per-path import filter (the union over every `import` of a path), the resolved modules a namespace reaches with `direct` re-scoped to it, and the data types it imports with every supplier — read by the checker's module registration and body checks and by code generation's per-module registrar and ADT membership, so a module's signatures and bodies are measured in the same namespace whichever file is the entry | `import_filters()`, `modules_visible_to()`, `imported_data_types()` |
...
| `codegen/` | 22,674 | Compile | Codegen orchestrator (mixin package) | `compile()`, `execute()` |
