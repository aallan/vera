...
- **Inline-refinement return types** in `_infer_fncall_vera_type` and `_infer_index_element_type_expr` — third trigger of the same bug class as #602 (i64/i32 mismatch at WASM validation) and the type-alias case fixed in v0.0.140.  Surfaced during PR #627's review (CodeRabbit duplicate-comment escalation, merged before the fix landed in #627 itself).
  ...
  Fix: extracted the i32_pair return-type resolution into a helper `_resolve_i32_pair_ret_te` that handles both `NamedType` (with alias resolution via `_resolve_base_type_name`) and `RefinementType` (recursive unwrap of arbitrary nesting depth, then resolve).  Applied to both i32_pair branches (non-generic + generic-mono) and to the parallel IndexExpr inference path.  Three new regression tests in `TestStringInterpolation` cover the inline-refinement String, nested-refinement, and refinement-over-alias cases plus the Array-indexed shape.

  PR review pass surfaced a fifth trigger of the same bug class — **nested refinements** (`@{ @{ @String | p1 } | p2 }`), reachable per the grammar.  The single-layer `if isinstance(...): unwrap` in the initial v0.0.141 fix fell through to None for nested forms.  Replaced with a `while isinstance(ret_te, ast.RefinementType): ret_te = ret_te.base_type` loop covering arbitrary nesting depth.  Same change in both call sites.  All five return-type shapes now verified — `f()` baseline (#602), type alias over String, inline refinement over String, nested refinement over String, refinement-over-alias — plus the parallel `IndexExpr`-of-`FnCall` path for refinement-`Array` returns.
