...
- **Soundness-core test hardening — contract-verifier discharge/report layer (first pass, [#387](https://github.com/aallan/vera/issues/387))**.  Extends the Phase 3 campaign from the type checker to `verifier.py`.  `TestObligationRecordCompleteness387` (23 tests) pins the obligation-record and diagnostic-content surface that the coarse `_verify_err` / `_verify_ok` helpers left unasserted — every `ProofObligation` field (`fn_name`, `error_code`, `counterexample`, `expr_text`) and the `_report_*` diagnostic content (`description` / `rationale` / `fix` / `spec_ref`, plus the `"modulo"`-vs-`"division"` branch) for `div_zero` (E526), `nat_sub` (E502), `index_bounds` (E527), `nat_bind` (E503), `refine_bind` (E505), and postcondition (E500) sites — plus the `@Float64`-divisor exemption guard and the verified / tier3 `summary` counter bookkeeping (`tier1_verified` / `tier3_runtime` / `total`) via exact-count assertions.  Drops the eleven primitive-safety and binding discharge/report functions from 413 to 313 mutation survivors.  Test-only; no behaviour change.  The issue stays open for the remaining per-branch tier3 coverage, the verifier walkers / `_verify_fn` spine, and `smt.py`.
