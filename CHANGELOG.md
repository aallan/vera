...
- **`codegen/api.py` decomposition — step 1: ADT memory layout ([#421](https://github.com/aallan/vera/issues/421))**.  Lifted the pure ADT memory-layout utilities (`ConstructorLayout`, `_validate_wrap_handle`, `_wasm_type_size` / `_wasm_type_align` / `_align_up`) out of the 4,358-line `vera/codegen/api.py` into a focused `vera/codegen/memory.py`, with the codegen consumers importing them from there directly.  The public surface is unchanged — `from vera.codegen import ConstructorLayout` and `from vera.codegen.api import compile` / `execute` / `CompileResult` / `ExecuteResult` all still resolve — and the #734 characterization harness stays green.  First of a per-concern series that lifts the runtime out of the public-API module so the deferred whole-`vera/` mutation sweep ([#387](https://github.com/aallan/vera/issues/387)) becomes tractable.  Internal refactor; no behaviour change.
