...
- **Host-side ADT result builders now root freshly-allocated string and array blocks across the enclosing struct/array allocation** ([#706](https://github.com/aallan/vera/issues/706)).  `_alloc_option_some_string` / `_alloc_result_ok_string` / `_alloc_result_err_string` / `_alloc_array_of_strings` (CLI) and `mapAllocOption` / `allocResultOkString` / `allocResultErrString` / `mapAllocArrayOfStrings` / `html_query` (browser) allocated a string, then the wrapping struct — a garbage collection triggered by the second allocation could sweep the still-host-local pointer and store a dangling reference, surfacing under `VERA_EAGER_GC=1` or heap pressure (e.g. `map_get` on a `Map<K, String>` returning a corrupted value).  A pre-existing gap the #692 / #695 rooting work left in these simpler builders; both runtimes now root via `_ShadowGuard` / the new JS `gcRooted` helper.  Surfaced by the CodeRabbit review.
