...
### Fixed
- **Active reclamation of host-store handles — closes [#573](https://github.com/aallan/vera/issues/573), [#575](https://github.com/aallan/vera/issues/575), [#576](https://github.com/aallan/vera/issues/576)**.  Pre-fix, every `map_new` / `map_insert` / `map_remove`, every `set_new` / `set_add` / `set_remove`, and every Decimal arithmetic op allocated a fresh entry in the corresponding Python-side store (`_map_store` / `_set_store` / `_decimal_store` in `vera/codegen/api.py`) and never released transient predecessors *within a single `execute()` call*.  A 10 000-iteration `array_fold` over `map_insert` left 10 001 entries in the store at `execute()` exit.  Each store is local to one `execute()`, so the leak doesn't accumulate across separate calls — but a single long-running call (a server's request loop running inside one `execute()`, an interactive session, a Game-of-Life-style program with many generations) could exhaust memory monotonically.
  ...
  **Tests** (`tests/test_codegen.py::TestHostHandleReclamation573`): seven regression tests covering all three types — chain-reclaims-transients (10K Map, 10K Set, 5K Decimal) plus value-correct-after-pressure (Map, Set, Decimal) plus the JObject case proving JSON/HTML internal Maps are reclaimed too.
