...
array_fold(
  ...
  0,
  fn(@Int, @Int -> @Int) effects(pure) {
    @Int.1 + @Int.0 + nat_to_int(array_length(@Array<Int>.1))  -- can't capture this
  }
)
-- Fails at WASM validation: "unknown table 0: table index out of bounds"
-- or "type mismatch: expected i32, found i64" depending on the exact shape.
```
...
```vera
-- BROKEN: nested closures fail at WASM validation.
let @Array<Array<Int>> = array_map(
  array_range(0, 3),
  fn(@Int -> @Array<Int>) effects(pure) {
    array_map(array_range(0, 3), fn(@Int -> @Int) effects(pure) { @Int.0 })
...
-- WORKING: inner array_map lives in a named helper.
private fn fill_row(@Int -> @Array<Int>)
  requires(true) ensures(true) effects(pure)
{
  array_map(array_range(0, 3), fn(@Int -> @Int) effects(pure) { @Int.0 })
...
|---|---|---|---|
| Nested closures | A closure directly nested inside another closure (`fn { array_map(..., fn { ... }) }`) fails at compile time; a closure capturing an outer scalar and passing it to a helper that uses `array_map` internally traps at runtime with `undefined element: out of bounds table access`. | Extract the inner closure into a named `private fn`. Pass captured scalars as explicit parameters to the helper rather than relying on capture. See [Closures and captured bindings → Known limitation](#known-limitation-nested-closures-and-captured-scalar-indirection) above. | [#514](https://github.com/aallan/vera/issues/514) |
...
When a Vera program type-checks cleanly, compiles without errors, and then produces a runtime trap you can't explain, the `undefined element` or i32/i64 mismatch class of failures is very likely [#514](https://github.com/aallan/vera/issues/514) surfacing in a new shape.
