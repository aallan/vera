...
| Bare `None` as a call argument | A `None` passed as a call argument is rejected (E202 in generic calls, E241 in `eq(...)` even at concrete types) — argument inference never feeds the expected type into a bare nullary constructor; same bucket: `MkA(None)` as `@A<T>` (bare forall var, E121) and both-ctor comparisons (`Some(5) == None`, E142). | Bind first (`let @Option<Int> = None;` and pass the slot), or use the operator form (`x == None`) for comparisons — both check clean. | [#993](https://github.com/aallan/vera/issues/993) |
