...
|-----------|---------|-------|
| Refinement runtime guards cover primitive bases only | A refinement type `{ @T \| P }` over a primitive base (`@Int`/`@Nat`/`@Bool`/`@Float64`/`@String`) is discharged **statically** at every narrowing site (a violation is `E505`) **and** runtime-guarded at the function boundary — a refined param is checked at entry, a refined return at exit, so an unverified compile traps on a violating value (a call argument is covered by the callee's entry guard). A refinement over a non-primitive base (e.g. `Array`) stays Tier-3 statically (`E506`) and is not refinement-runtime-guarded; the underlying operation's checks (e.g. array bounds) apply instead. | [#762](https://github.com/aallan/vera/issues/762) |
