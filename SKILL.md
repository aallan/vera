...
| `Nat` component in a partially-generic ctor argument | A negative value stored into a concrete `Nat` component of a constructor argument whose parameter type also carries a type variable (`MkPair(0 - 5, None)` as `@Pair<Nat, T>`) verifies **clean** and silently stores the negative — the E503 obligation is lost. | Bind the component first (`let @Nat = <value>;` and pass the slot) so the let-binding's own narrowing obligation fires, or make the parameter fully concrete. | [#1010](https://github.com/aallan/vera/issues/1010) |
