...
| Metric | Value |
|--------|-------|
| **Tests** | 4,571 across 37 files (~59,400 lines of test code; 4,540 passed + 16 stress, 15 skipped) |
...
| `test_monomorphize_differential.py` | 8 | 325 | #732 differential soundness: the verifier's per-monomorphization instantiation discovery covers every instantiation codegen emits (name coverage + per-generic count), over real generic programs (conformance ch02/ch09, `examples/generics.vera`) plus inline cases for the soundness-critical scenarios — collapsed type vars, **prelude combinator emission** (`option_map`), transitive generics, and a generic whose type arg is fixed only by a **where-helper's return** (a `Float64`-returning helper, so the unresolved-var `"Bool"` phantom default cannot mask a miss) — so a missed instantiation (a false Tier-1) is caught. Guards against a vacuous pass when codegen emits nothing |
