...
| Metric | Value |
|--------|-------|
| **Tests** | 3,217 across 27 files (~34,000 lines of test code) |
...
# Validation scripts
python scripts/check_conformance.py                  # conformance suite (72 programs, see manifest.json)
...
| `test_cli.py` | 216 | 3,010 | CLI commands (check, verify, compile, run, test, fmt, version, quiet), subprocess integration, JSON error paths, runtime traps, arg validation, multi-file resolution, IO exit codes, --explain-slots |
...
├── ...                        # 72 programs total, organized by spec chapter
...
```bash
# Via pytest (parametrized — 360 tests)
...
|--------|-------------------|
| `check_conformance.py` | All 72 conformance programs pass their declared level (parse/check/verify/run) |
