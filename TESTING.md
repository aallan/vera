...
| Metric | Value |
|--------|-------|
| **Tests** | 7,167 across 109 files (~94,000 lines of test code; 7,074 passed + 26 stress, 67 skipped) |
| **Compiler code coverage** | 95% Python, 61% JavaScript — 91% combined (CI minimum: 80%) |
| **Conformance programs** | 151 programs across 9 spec chapters, validating every language feature |
...
# Validation scripts
python scripts/check_conformance.py                  # conformance suite (151 programs, see manifest.json)
...
| `test_xmod_span_collision.py` | 3 | 123 | #987 — the span-keyed target-type table is single-module: an imported body's expression span can coincide with a main-file entry and pick up the wrong target type, so span-keyed recovery is suppressed for imported decls (`_compile_fn(imported=True)`) — an engineered line-for-line collision pair pins that a legal all-`@Nat` imported function is not falsely widen-guarded, with a same-file control proving top-level guards unaffected |
...
| `test_conformance.py` | 755 | 124 | Parametrized conformance suite: parse, check, verify, run, format idempotency across 151 programs |
...
| `run` | Compiles to WASM and executes correctly | 123 |
...
├── ...                        # 151 programs total, organized by spec chapter
...
|--------|-------------------|
| `check_conformance.py` | All 151 conformance entries hold at their declared level (parse/check/verify/run) — positives pass; the negatives fail `check` with their `expected_error` E-code |
...
| `check_conformance.py` | All 151 conformance entries hold at their declared level — positives pass; negatives fail `check` with their `expected_error` E-code |
