...
| Metric | Value |
|--------|-------|
| **Tests** | 4,945 across 41 files (~62,000 lines of test code; 4,913 passed + 16 stress, 16 skipped) |
...
| `test_float64_fp.py` | 6 | 145 | #797 — `@Float64` contracts via Z3's IEEE-754 FloatingPoint sort: unsound relational / reflexive contracts (rounding at 2^53, `NaN`, `Inf`) flip from proved to violated/Tier-3, NaN-guarded contracts still verify at Tier 1, `==`/`!=` use IEEE `fpEQ`/`fpNEQ`, `%` matches codegen `fmod` (not `fp.rem`), and `float_is_nan` / `float_is_infinite` / `nan()` / `infinity()` translate to FP predicates / constants. Test-first: each fails on the pre-fix Real-sort verifier |
