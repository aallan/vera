...
| Metric | Value |
|--------|-------|
| **Tests** | 5,768 across 90 files (~64,000 lines of test code; 5,702 passed + 26 stress, 40 skipped) |
...
| `test_codegen_gc_rooting.py` | 32 | 1,443 | Opaque-handle param rooting (#347, #490), host-walker GC rooting (#692), Map host-store reachability (#695), ADT-builder rooting (#743) (#419 split); plus the #841 Future-handle battery (`TestFutureHandleGCRooting841`) — eager-GC survival across an intervening alloc, the operand-stack window (`both(async(A), async(B))` with get/post-distinguished Err text), Phase-2c reclamation of fire-and-forget futures via `host_store_sizes["future"]`, and repeated-await memoization; and host-import pair-let rooting (`TestHostImportPairLetRooting846`) — `IO.args` / `IO.read_line` pairs surviving an intervening alloc under eager GC, with `IO.read_file` / `IO.get_env` ADT-path confirmation |
...
| Ch 13: WASI | WASI Preview 2 target (component emission, wasip2 host runner, dual-target differential) | test_wasi_target | run-level suite via the dual-target differential | — |
