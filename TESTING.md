...
| Metric | Value |
|--------|-------|
| **Tests** | 7,276 across 114 files (~94,000 lines of test code; 7,181 passed + 26 stress, 69 skipped) |
| **Compiler code coverage** | 95% Python, 61% JavaScript — 91% combined (CI minimum: 80%) |
| **Conformance programs** | 155 programs across 9 spec chapters, validating every language feature |
...
# Validation scripts
python scripts/check_conformance.py                  # conformance suite (155 programs, see manifest.json)
...
| `test_xmod_generic_widen_gap.py` | 3 | 158 | PR #997 review — honest pin of a PRE-EXISTING gap (out of scope for #987, tracked separately): an imported **generic** function's mono clones compile on the monomorphization path WITHOUT the threaded module table, so a concrete `Array<Int>` widening inside a `forall<T>` import is NOT runtime-guarded at any instantiation reached through the import door.  Pins that the standalone generic library (once instantiated) promises Tier-3, yet the importer silently reinterprets `u64.MAX` to `-1` at both `T=Bool` and `T=Int` clones — no trap — while in-range values pass; when the gap closes these runs will TRAP and the pin must flip (style of the #985 nested-closure honest pin) |
...
| `test_conformance.py` | 775 | 124 | Parametrized conformance suite: parse, check, verify, run, format idempotency across 155 programs |
...
| `run` | Compiles to WASM and executes correctly | 126 |
...
├── ...                        # 155 programs total, organized by spec chapter
...
|--------|-------------------|
| `check_conformance.py` | All 155 conformance entries hold at their declared level (parse/check/verify/run) — positives pass; the negatives fail `check` with their `expected_error` E-code |
...
| `check_conformance.py` | All 155 conformance entries hold at their declared level — positives pass; negatives fail `check` with their `expected_error` E-code |
