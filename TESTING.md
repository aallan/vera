...
| Metric | Value |
|--------|-------|
| **Tests** | 7,403 across 120 files (~104,000 lines of test code; 7,307 passed + 26 stress, 70 skipped) |
...
| `test_cli.py` | 263 | 4,109 | CLI commands (check, verify, compile, run, serve, test, fmt, version, quiet), subprocess integration, JSON error paths (including the `verify --json` `obligations` array and its summary-reproducibility pin, #967), runtime traps, arg validation, multi-file resolution, IO exit codes, --explain-slots, `builtins`/`effects`/`errors` introspection dispatch, and a USAGE-completeness guard (every dispatched `cmd_<name>` handler has a help row) |
