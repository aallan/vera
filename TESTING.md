...
| Metric | Value |
|--------|-------|
| **Tests** | 5,500 across 51 files (~63,000 lines of test code; 5,452 passed + 26 stress, 22 skipped) |
...
| `test_diagnostic_fields.py` | 36 | 435 | Unit tests for `scripts/check_diagnostic_fields.py` (#682) — required-field detection, the warning severity rule (no `fix`), the codegen structural-exemption registry, the `# diag-fields-exempt` per-call opt-out, plumbing-skip, and a live-tree integration check that all of `vera/` is fully tagged |
