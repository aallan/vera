...
| Metric | Value |
|--------|-------|
| **Tests** | 7,045 across 106 files (~94,000 lines of test code; 6,952 passed + 26 stress, 67 skipped) |
| **Compiler code coverage** | 95% Python, 61% JavaScript — 91% combined (CI minimum: 80%) |
| **Conformance programs** | 150 programs across 9 spec chapters, validating every language feature |
...
| **Contract verification** | 282 of 380 obligations (74.2%) across the 37 examples verified statically (Tier 1) — the denominator grew with the auto-synthesised primitive-op obligations of the soundness campaign |
...
# Validation scripts
python scripts/check_conformance.py                  # conformance suite (150 programs, see manifest.json)
...
| `test_verifier_adt_decreases.py` | 20 | 651 | Match/ADT verification, decreases measures (incl. ADT decreases), mutual recursion (#839 split) |
...
| `test_nat_narrowing_return_differential.py` | 5 | 166 | #758 verifier↔codegen behavioural differential (cross-component soundness rule): at the function-return `@Int -> @Nat` coercion slot the verifier's `nat_bind` verdict must AGREE with the runtime — an unproven narrowing leaves the return obligation undischarged (loud E503, or an honest `tier3` for an opaque value) and codegen's return guard TRAPS on a negative input, while a proven narrowing (`requires` / path condition) discharges at Tier 1 and the guard is dead (`vera run` returns the value, no trap).  Runs BOTH sides on one corpus so "the verifier obligates this return" is checked against the actual guard — the return-position dual of `test_int_widening_differential` |
...
| `test_codegen_nat_guards.py` | 45 | 1,153 | **`@Nat` runtime guards**: subtraction underflow (#520) and binding-site narrowing (#552 let site; #747 tuple-destructure / match-bind / ADT sub-pattern / ctor-field / call-arg sites — `i64.lt_s; unreachable` net, `@Int` targets exempt) (#419 split) |
...
| `test_cli.py` | 255 | 3,888 | CLI commands (check, verify, compile, run, serve, test, fmt, version, quiet), subprocess integration, JSON error paths (including the `verify --json` `obligations` array and its summary-reproducibility pin, #967), runtime traps, arg validation, multi-file resolution, IO exit codes, --explain-slots, `builtins`/`effects`/`errors` introspection dispatch, and a USAGE-completeness guard (every dispatched `cmd_<name>` handler has a help row) |
...
| `test_conformance.py` | 750 | 124 | Parametrized conformance suite: parse, check, verify, run, format idempotency across 150 programs |
...
| `run` | Compiles to WASM and executes correctly | 122 |
...
├── ...                        # 150 programs total, organized by spec chapter
...
| **Total** | 378 obligations (74.9% static; the summary's `total` field equals `tier1_verified + tier3_runtime`, derived from the reified obligation stream) |
...
|--------|-------------------|
| `check_conformance.py` | All 150 conformance entries hold at their declared level (parse/check/verify/run) — positives pass; the negatives fail `check` with their `expected_error` E-code |
...
| `check_conformance.py` | All 150 conformance entries hold at their declared level — positives pass; negatives fail `check` with their `expected_error` E-code |
