...
| Metric | Value |
|--------|-------|
| **Tests** | 7,490 across 122 files (~104,000 lines of test code; 7,394 passed + 26 stress, 70 skipped) |
...
| `test_refined_closure_return_differential.py` | 4 | 209 | #1032 verifier↔codegen behavioural differential — the refinement dual of `test_nat_narrowing_return_differential`.  A closure whose declared return is a refinement `{ @Base \| P }` (e.g. `Pos = { @Nat \| @Nat.0 > 0 }`) returning a raw `@Base` value that violates `P` verified clean (a false Tier 1) and returned the violating value through the refined slot with no trap.  The opaque closure body forces a shallow-syntactic `tier3` obligation on the `AnonFn` refined return (never a Tier 1, alongside the #820 widen / #984 narrow arms) and codegen guards the closure return value in `_compile_lifted_closure` (the closure analogue of the named-function `_compile_postconditions` guard).  Runs BOTH sides on one corpus: the return `refine_bind` is `tier3`, a strictly-negative value (violating `>= 0` and `> 0`) AND a zero (violating only the strict `> 0`) both trap at the closure boundary, a satisfying value passes, and a non-refined `@Int -> @Int` closure return carries no obligation and no guard.  A refined PAIR-base return (`{ @String \| string_length > 0 }`) is guarded too — codegen saves/reloads the `(ptr, len)` halves and checks the predicate on the ptr, mirroring the named-function i32_pair return guard — so an empty string traps while a non-empty one passes |
