...
| Metric | Value |
|--------|-------|
| **Tests** | 5,554 across 52 files (~63,000 lines of test code; 5,506 passed + 26 stress, 22 skipped) |
...
| `test_check_explicit_encoding.py` | 51 | 250 | `check_explicit_encoding.py` gate (#645): flags text-mode `open()` / `read_text()` / `write_text()` **and** `subprocess.run/Popen/check_output(..., text=True)` captures missing an `encoding="utf-8"` literal (rejects non-literal / non-UTF-8 values), skips binary/bytes-mode calls, honours the `# encoding-exempt` opt-out, and asserts the shipped repo is clean |
