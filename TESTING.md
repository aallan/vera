...
| Metric | Value |
|--------|-------|
| **Tests** | 3,369 across 28 files (~34,000 lines of test code; 3,356 passed, 13 skipped) |
| **Compiler code coverage** | 96% of 15,149 statements (CI minimum: 80%) |
| **Conformance programs** | 76 programs across 9 spec chapters, validating every language feature |
...
# Validation scripts
python scripts/check_conformance.py                  # conformance suite (76 programs, see manifest.json)
...
| `test_browser.py` | 67 | 865 | Browser parity: Python/wasmtime vs Node.js/JS-runtime output equivalence across IO, State, contracts, Markdown, Regex, and all compilable examples |
| `test_conformance.py` | 380 | 102 | Parametrized conformance suite: parse, check, verify, run, format idempotency across 76 programs |
...
| `verify` | Type-checks and all contracts verified by Z3 | 5 |
| `run` | Compiles to WASM and executes correctly | 67 |

Almost all programs are at the `run` level — they compile and execute, producing correct results. Four programs (`ch07_cross_module_contracts_lib`, `ch09_http`, `ch09_inference`, `ch03_typed_holes`) are at the `check` level. Five programs (`ch03_slot_let_chains`, `ch03_slot_noncommutative`, `ch07_cross_module_contracts`, `ch07_io_sleep`, `ch07_random_effect`) are at the `verify` level, using Z3-provable contracts.

### Skipped tests

`pytest tests/ -v` reports 13 skipped tests across two categories:
...
├── ...                        # 76 programs total, organized by spec chapter
...
|--------|-------------------|
| `check_conformance.py` | All 76 conformance programs pass their declared level (parse/check/verify/run) |
...
| `check_conformance.py` | All 76 conformance programs pass their declared level |
