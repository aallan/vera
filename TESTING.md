...
| Metric | Value |
|--------|-------|
| **Tests** | 4,570 across 37 files (~59,400 lines of test code; 4,539 passed + 16 stress, 15 skipped) |
...
| `test_monomorphize_differential.py` | 7 | 270 | #732 differential soundness: the verifier's per-monomorphization instantiation discovery covers every instantiation codegen emits (name coverage + per-generic count), over real generic programs (conformance ch02/ch09, `examples/generics.vera`) plus inline cases for the soundness-critical scenarios — collapsed type vars, **prelude combinator emission** (`option_map`), and transitive generics — so a missed instantiation (a false Tier-1) is caught. Guards against a vacuous pass when codegen emits nothing |
