...
| Metric | Value |
|--------|-------|
| **Tests** | 5,525 across 52 files (~63,000 lines of test code; 5,477 passed + 26 stress, 22 skipped) |
...
| `test_cli.py` | 242 | 3,506 | CLI commands (check, verify, compile, run, test, fmt, version, quiet), subprocess integration, JSON error paths, runtime traps, arg validation, multi-file resolution, IO exit codes, --explain-slots, `builtins`/`effects`/`errors` introspection dispatch |
...
| `test_check_explicit_encoding.py` | 22 | 136 | `check_explicit_encoding.py` gate (#645): flags bare text-mode `open()` / `read_text()` / `write_text()` calls missing `encoding="utf-8"`, skips binary-mode calls, honours the `# encoding-exempt` per-call opt-out, and asserts the shipped repo is clean |
