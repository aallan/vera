...
| Metric | Value |
|--------|-------|
| **Tests** | 5,530 across 52 files (~63,000 lines of test code; 5,482 passed + 26 stress, 22 skipped) |
...
| `test_check_explicit_encoding.py` | 27 | 154 | `check_explicit_encoding.py` gate (#645): flags bare text-mode `open()` / `read_text()` / `write_text()` calls missing `encoding="utf-8"`, skips binary-mode calls, honours the `# encoding-exempt` per-call opt-out, and asserts the shipped repo is clean |
