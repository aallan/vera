...
Four pure functions for pattern matching on strings using regular expressions. All accept patterns in standard regex syntax and return `Result` types to safely handle invalid patterns. Every failure of a runtime's regex engine on the inputs is that `Err` and never a trap: a malformed pattern, a repetition bound past the engine's limit, a pattern nested deeper than the engine compiles, and a replacement naming a group the pattern lacks. The two runtimes' engines are different regex dialects, so which patterns a runtime accepts, and its message for one it refuses, are the engine's own and are not pinned.
...
**Nesting depth.** RFC 8259 §9 lets an implementation set a maximum nesting depth, and Vera's is 512: text whose arrays and objects nest more than 512 levels deep MUST produce `Err`, with the same message on every runtime. Depth is measured on the text before it is parsed, by one rule on every runtime: `[` and `{` outside a string literal open a level, `]` and `}` close one (never below zero), and a backslash inside a string literal skips the next character. So a text that is both too deep and malformed reports its depth. The limit is Vera's, not a host parser's; without it the same text parsed on one runtime and ended the program on the other. `json_stringify` has no depth limit: it serializes a `Json` value of any depth built in Vera.
...
**Nesting depth.** Text whose blocks and inline spans nest more than 512 levels deep MUST produce `Err`, with the same message on every runtime. A level is each block quote, list item, link, emphasis or strong span enclosing a construct; a paragraph's, heading's or table cell's inline content sits at its block's own depth. Every runtime's parser counts the same levels, so the limit is Vera's, not a host's. `md_render` and the three queries have no depth limit: they accept a tree of any depth built in Vera.
...
Parses an HTML string into an `HtmlNode` tree. The parser is lenient (like browsers) — malformed HTML produces a best-effort tree rather than an error. Returns `Err` only on catastrophic parse failures, and every failure of the host parser is that `Err`, never a trap. `html_to_string`, `html_text` and `html_query` accept a tree of any depth built in Vera.
