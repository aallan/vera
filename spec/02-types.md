...
The conjunction is what every rule in §2.6.4 is stated about.  It is the goal discharged at each narrowing site, the predicate an `E505` names, and the fact a refined **return type** publishes to a caller as an implicit postcondition.  Whether the refinement is one the verifier models is decided at the **primitive the chain bottoms out in**, never at an intermediate refinement — so a chain over `@Int` is modelled, a chain over `@Byte` is Tier 3 for cause 2 of §2.6.4, and a **concrete** narrowing into either is decided by substitution exactly as it is for a single refinement.  A chain with one level is that single refinement, so the rule subsumes it.
