...
A refinement's base MAY itself be refined.  `type Small = { @Pos | @Pos.0 < 10 }` over `type Pos = { @Int | @Int.0 > 0 }` is a **refinement chain**, and membership in it is the **conjunction of every predicate in the chain, applied to the same value**: a `Small` is an `Int` that is both `> 0` and `< 10`.  Each level binds its own name — the inner predicate is written about `@Int`, the outer about `@Pos` — and both denote the one value being narrowed, so the levels are conjoined rather than composed.  The base's intrinsic invariant is contributed **once for the whole chain**, not once per level: a chain bottoming out in `@Nat` carries `>= 0` a single time.

The conjunction is what every rule in §2.6.4 is stated about.  It is the goal discharged at each narrowing site, the predicate an `E505` names, and the fact a refined **return type** publishes to a caller as an implicit postcondition.  Whether the refinement is one the verifier models is decided at the **primitive the chain bottoms out in**, never at an intermediate refinement — so a chain over `@Int` is modelled, a chain over `@Byte` is Tier 3 for cause 2 of §2.6.4, and a **concrete** narrowing into either is decided by substitution exactly as it is for a single refinement.  A chain with one level is that single refinement, so the rule subsumes it.  The base is one of two conditions: the refinement is modelled only while its whole predicate — every level's, conjoined — can be **stated** to the solver.  A level that calls something outside the decidable fragment (cause 3 of §2.6.4) leaves the value carried by a sort with nothing said about it, which is worse than no sort at all: a counterexample may then name a value the type forbids.  Such a refinement is not modelled, the obligations over it are Tier 3, and the runtime guard behind each of them is what holds.
