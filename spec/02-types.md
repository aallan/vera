...
- a refinement written on a **component** of a type — an ADT payload, a tuple component — at every position that gives a value that type: a call argument, a `let` binder, a constructor field, a tuple component, an effect-operation argument, and the function's return position
...
That assumption extends to refinements written *inside* the parameter's type: a `consume(@Option<PosInt>)` reasons about its payload as `> 0` without re-deriving it, and it is verified once for every caller, so the caller must establish the payload. The obligation is therefore raised at **every position that gives a value a type whose components are refined** — the argument, a `let` binder, a constructor field, a tuple component, an effect-operation argument, and the function's own return slot. Covering all of them is what makes the rule closed rather than local: a value cannot acquire the declared type anywhere without being asked for it, so no position can publish a component invariant it did not establish, and a chain of wrappers cannot launder one. The single exclusion is a value **constructed** here, whose own construction site carries the obligation; it holds only when every value-producing leaf constructs, since a branch join with one constructed arm establishes nothing about the other.
