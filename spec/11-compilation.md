...
The compiled file's `public` top-level functions are exported from the WASM module under their source names. Where-block functions are internal (not exported). The runtime's own exports are in the `vera.` namespace (§12.2.1), so a program's export can never collide with one.

### 11.4.5 Symbol Ownership

Every function in the WASM module has exactly one owner, and its symbol says which. There are four kinds of owner, and one function namespace they all share:

| Owner | Symbol |
|-------|--------|
| the compiled file (the entry) | its source name, `$name` |
| an imported module | `$name` when the entry's bare name denotes it (§8.5.2.1); otherwise `$<path>::name`, the path spelled as in source (`$a.b::name`) |
| the prelude | `$name`, unless the entry declares its own function of that name — then `$<prelude>::name` |
| the runtime (allocator, collector, derived helpers, lifted closures) | `$rt.<name>`, always |

A function derived from a declaration extends that declaration's symbol: a hoisted `where` helper `h` of `f` is `$f$where$h` (`$a.b::f$where$h` for a module's), and a monomorphized clone appends its escaped type arguments (`$gen$Int`). One encoder builds every such symbol and one decoder reads it back, and no two constructions can spell the same symbol: `::` and `.` appear in no identifier, path segment or escaped type, `<` begins no path segment, and `where` is reserved, so `$where$` can never be a clone's type argument.

Every call is compiled to the symbol of the declaration the checker resolved it to, in its OWN namespace (§8.5.2.1): the entry's bare calls to its own declarations and imports, a module's to its declarations, its imports and the prelude, and the prelude's own bodies to the prelude's functions. The checker records each call's target, and code generation and the verifier bind to that record rather than deriving a second answer, so a `where` helper binds only inside its parent's body, whatever the parent's genericity. A program may therefore name a function after anything the runtime or the prelude defines. An entry function of a prelude function's name takes the program's bare name, and the prelude's bodies — and every module that does not declare the name — go on calling the prelude's. A symbol no Vera declaration can spell (`$`, `.`, `:` and `<` are not identifier characters) cannot collide with a declaration, and because the entry keeps its source names, `vera run --fn`, the browser runtime's `call()` and the wasi-p2 component's entry points name functions exactly as the program wrote them.
