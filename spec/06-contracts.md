...
Stage 2 is not merely sound but **refutation-complete** as the verifier stands.  The only quantified premises it ever drops are the rank axioms a `decreases` measure installs, and the symbol those introduce occurs in no other premise — so a premise set is an ordinary part plus a definition of a fresh function, and any model of the ordinary part extends to one of the whole.  Dropping them therefore loses no refutation.  What would change that is a quantified premise that is not a rank axiom, or a rank symbol occurring in an ordinary premise; both conditions hold at this revision by measurement.  Should either stop holding, the residual becomes real: a contradiction refutable only through a quantified fact that does not propagate inside the first stage's budget would go undetected.

An UNSAT precondition is a warning rather than an error: the tiers this chapter defines describe how much was established, and "nothing was established" is a tier, not a malformed program.  The call sites answer for it separately where they can: a call to a MONOMORPHIC function whose precondition has no model reports the ordinary `E501` and the program is refused.  Through a `forall` generic callee the same call is only disclosed — `call_pre` at Tier 3, `E532` — and the program is accepted; that gap is [#1468](https://github.com/aallan/vera/issues/1468) and is not repaired here.
