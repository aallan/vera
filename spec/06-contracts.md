...
Only a REFUTATION demotes.  An undecided satisfiability check has established nothing, and demoting a function on an expired budget would turn a slow solver into a wrong verdict — the distinction §6.8 draws everywhere else between "could not decide" and "decided against".  This is not a corner: a `decreases` measure over a recursive ADT installs quantified rank axioms, and the full premise set of such a function is routinely undecided while a *contradictory* one in the same context is refuted at once, before any quantifier is instantiated.
