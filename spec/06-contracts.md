...
| **unguarded** | `tier3_unguarded` | Neither proved nor guarded. Counted in no tier, and reported as a warning (`E504`, `E506`, `E531`, `E539`) — or, for `E538`, as an error that refuses the program (§6.8.2). |
...
Stage 2 is not merely sound but **refutation-complete** as the verifier stands.  The only quantified premises it ever drops are the rank axioms a `decreases` measure installs: two constraints on one fresh function, satisfiable over any model of the datatype sort, with structural depth as the witness.  The symbol they introduce occurs in no other premise, so any model of the quantifier-free part extends to a model of the whole by interpreting that symbol, and dropping the axioms loses no refutation.  The condition the argument rests on — that the quantified and quantifier-free halves of a premise set share no uninterpreted symbol — is ASSERTED over the corpus rather than left to hold by inspection, so it cannot lapse in silence.

Any future quantified premise reopens the design of this screen.  A Tier 2 hint ([#427](https://github.com/aallan/vera/issues/427)) is the live candidate: a quantified fact that is not a rank axiom, or a rank symbol occurring in an ordinary premise, makes the residual real — a contradiction refutable only through a quantified fact, that does not propagate inside the first stage's budget, would go undetected.
