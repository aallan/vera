...
One residual is left, and stated rather than hidden: a contradiction that can be refuted ONLY through a quantified fact, and that does not propagate inside the first stage's budget, is not detected.  Such a function keeps its tier counts.  No program is known that reaches it.
