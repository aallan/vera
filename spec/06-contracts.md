...
- **Preconditions stay strict.**  A call precondition that cannot be proven — because the argument is opaque — is reported (**E501**), exactly as for an opaque function result: establishing the precondition is the caller's obligation, and the repair is an `assert`/`assume` on the value, whose fact flows into the check ([#804](https://github.com/aallan/vera/issues/804)).  This holds wherever an `assume` can reach the value, however deeply the call is nested: at the top of a `let`'s body, in an argument of a built-in the verifier does not model, in an interpolated part, in a pipe, and in a `match` arm whose binder is a value the verifier cannot know because the scrutinee does not translate.  It does not hold where no `assume` can reach the value: a call precondition over a binder of a closure, of a quantifier's predicate or of a handler clause, which the verifier reads without its value, takes the **E532** Tier-3 demotion above, as an argument that does not translate does, with the callee's own check behind it ([#1480](https://github.com/aallan/vera/issues/1480)).
...
| **runtime-guarded** | `tier3`, `timeout` | Tier 3. Not proved, but the compiler emitted a guard that traps on violation. Counted in `tier3_runtime`. A call precondition over a binder of a closure, a quantifier's predicate or a handler clause lands here (`E532`, Section 6.4.2): no `assume` can reach it. |
...
| **refuted or unprovable** | `violated` | The obligation did not discharge and the compiler refuses the program. Two ways in: Z3 returned a concrete counterexample, or — for a call precondition over an opaque value an `assume` could reach, however deeply the call is nested (Section 6.4.2) — it could not establish the goal at all. Both report `violated`, which is why the diagnostic says a call *may* violate the precondition rather than that it does. A compile error (`E500`, `E501`, `E502`, `E505`, …), counted in no tier. |
