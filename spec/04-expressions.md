...
**An integer literal takes its type from its context.** Where the context names a type — a `let` binding, a parameter, a return, a constructor field — the literal is checked at that type. Where nothing names one, a non-negative literal is a `Nat` and a negative one an `Int`. An expression built from integer literals alone with `+`, `-`, `*`, `/`, `%` and negation is typed the same way, by its value: `0 - 3` and `(1 - 4) * 2` are `Int`, `5 - 2` is `Nat`.
...
2. the type the result is expected at: with `forall<T> fn id(@T -> @T)`, `let @Int = id(5)` instantiates `id` at `Int`;
...
A literal whose context turns out to be `Nat` is checked for non-negativity like any other. With `forall<T> fn pick(@T, @T -> @T)`, `pick(@Nat.0, 0 - 1)` has `T` fixed at `Nat` by the `@Nat` argument, and `0 - 1` is a narrowing the verifier refutes (**E503**); so is `let @Nat = id(0 - 3)`.
