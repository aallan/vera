...
| The `@Nat >= 0` narrowing **runtime** guard covers every concrete binding site, generic function-formal calls, and builtin `@Nat` parameters, but two sites stay runtime-unguarded in an unverified compile: the effect-operation argument (codegen's `_effect_ops` carries only the dispatch target, not the op's formal types) and the generic-instantiated constructor field (constructor layouts carry no per-field `@Nat` mono metadata). Both are statically obligated by the verifier, so verified programs are sound (#747), and a tripped guard surfaces a generic `unreachable` rather than the `requires(... >= 0)` fix. | [#754](https://github.com/aallan/vera/issues/754), [#757](https://github.com/aallan/vera/issues/757) |
