...
| A type argument inferred from a literal subtraction is `Nat`, because the checker types `0 - 3` bottom-up: `let @Int = id(0 - 3)` is refused with E503 and traps on the widening guard, and `get_nat(id(W(0 - 3)))` verifies clean and traps where `W(@Nat)` binds the -3. | [#1541](https://github.com/aallan/vera/issues/1541) |
