...
| General refinement-type predicates (`{ @T \| P }`) are verified **statically** at every narrowing site (`let`, call argument, constructor field, effect-op argument, match binding, tuple-destructure component) and at refined return positions — a provably-violating narrowing is an E505 error and an untranslatable one is an E506 Tier-3 warning, so a `vera verify`-clean program admits no predicate-violating value at these sites (#746). The compiler does **not** yet emit a *runtime* guard for a predicate it cannot discharge statically (untranslatable predicate, non-primitive base such as `{ @Array<Int> \| array_length(...) > 0 }`, or solver timeout), so such a Tier-3 narrowing — including at a `public`/FFI boundary — is neither statically proved nor runtime-checked. No statically-verified program is weakened; this only bounds the Tier-3 fallback. The refinement-predicate analogue of the `@Nat` runtime-guard residual (#754/#757). | [#762](https://github.com/aallan/vera/issues/762) |
| Generic (`forall<T>`) functions skip the E502 underflow obligation at verify time, because the verifier's early return for generics skips all static checks — this also means a concrete refined return on a generic function (`forall<T> fn f(@T -> @PosInt)`) is not obligated, the #746 refinement-predicate sub-case. The codegen guard still fires per monomorphization; per-monomorphization static verification (#732) closes the static half. | [#555](https://github.com/aallan/vera/issues/555) |
