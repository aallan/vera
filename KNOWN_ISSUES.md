...
| Bug | Issue |
|-----|-------|
| A value whose refinement the solver cannot STATE — a predicate calling something outside the decidable fragment, spec §2.6.4 cause 3 — is modelled with nothing said about it, so a refutation can rest on a value the type forbids.  `100 / @U.0` over `type U = { @Int | int_to_float(@Int.0) > 0.0 && @Int.0 > 0 }` is refused E526 with the divisor 0 at a parameter, a refined return and a type argument alike, on a program that runs and returns 25. | [#1470](https://github.com/aallan/vera/issues/1470) |
