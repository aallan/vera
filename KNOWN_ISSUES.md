...
| Wrap-table overflow traps without trying compaction first (#573 follow-up). `$register_wrapper` traps with `unreachable` when the wrap-table is full (4 096 simultaneously-live entries) instead of triggering `$gc_collect` first to compact dead entries via Phase 2c. In practice the trap is bounded by heap fill rate (every wrapper is also a heap allocation, so wrap-table-full and heap-full happen at similar cadences and GC fires first); no reported program has hit the trap. Fix is a two-step register: on overflow, root the new wrapper, call `$gc_collect`, re-check, only trap if still full after compaction. | [#579](https://github.com/aallan/vera/issues/579) |
