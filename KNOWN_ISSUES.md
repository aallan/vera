...
| macOS malloc abort during wasmtime cleanup after Ctrl-C arrives in a host import (observed with `IO.sleep`).  The Python-side `KeyboardInterrupt` traceback half is fixed in v0.0.137 (host_sleep converts to `_VeraExit(130)` for clean exit), but a follow-on `pointer being freed was not allocated` / `Abort trap: 6` from the wasmtime/ctypes teardown path can still fire if the program was in certain heap states.  Possibly shares a root cause with [#593](https://github.com/aallan/vera/issues/593) — both surface together when running Conway's Life and pressing Ctrl-C.  No data-integrity impact; only the cleanup path. | [#595](https://github.com/aallan/vera/issues/595) |
| Conway's Game of Life at 12×30 grid scale (200 generations, recursive `run_loop` with allocating `next_grid` argument across the recursive call boundary) silently corrupts rendered string output from generation 1 onwards. Generation 0 renders cleanly, then U+FFFD replacement characters interleave with valid block characters and ANSI escape bytes are partially stripped. The bisection narrows the trigger to: 12×30 scale + recursive run_loop with allocating call argument; smaller scales (~5×5) or non-recursive sweeps work cleanly. Hypothesis: GC reclamation of in-flight strings during `string_join` / `string_concat` while the destination buffer is still being filled. The v0.0.136 `errors="replace"` defensive layer (#589) prevents this from escaping as a Python traceback — corruption is visible as U+FFFD chars in output but the program continues running. Acknowledged as an unisolated "additional trigger" in the original [#588](https://github.com/aallan/vera/issues/588) bug report. | [#593](https://github.com/aallan/vera/issues/593) |
