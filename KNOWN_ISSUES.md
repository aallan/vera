...
| Closure capturing a `data` ADT and passing it as an argument to a function call inside the closure body emits malformed WASM (`unknown table 0: table index out of bounds` at module instantiation).  `vera check` is silent.  Single-variant ADTs reproduce identically; capture without function call is fine.  Workaround: pre-extract the inner field outside the closure and capture the field, not the ADT.  Discovered via the Tetris experiment. | [#614](https://github.com/aallan/vera/issues/614) |
| Non-contiguous capture of outer same-typed slots in a closure miscompiles.  If a closure references captured outer `@Int.k` while leaving some `@Int.j` (with `0 < j < k`) unreferenced, the WASM is malformed: tail-shaped expressions trap at instantiation (`unknown table 0: table index out of bounds`); body shapes that push more `let` slots after the skip compile and run but silently produce the wrong value.  `vera check` is silent in both cases.  Hypothesised same root cause as #614 in the closure-lifter's env-struct serialisation.  Workaround: hoist the closure body into a top-level helper that takes all outer values as ordinary arguments — closure body becomes a single helper call, slot capture becomes contiguous.  Discovered via the Tetris experiment. | [#615](https://github.com/aallan/vera/issues/615) |
