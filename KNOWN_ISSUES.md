...
| The `@Nat >= 0` narrowing **runtime** guard now covers every binding site except the effect-operation argument — `let`, call argument, concrete constructor field, top-level match bind, ADT sub-pattern, and tuple destructure all trap on a negative `@Nat` in an unverified compile, but `E.op(@Int.0)` does not, because codegen's `_effect_ops` carries only the dispatch target, not the op's formal types; and a tripped guard surfaces a generic `unreachable` rather than the `requires(... >= 0)` fix. Every site (including the effect-op argument) is statically obligated by the verifier, so verified programs are sound (#747); closing the residual needs only codegen plumbing — an op-parameter registry and the contract-fail host channel wired to a mid-expression guard. | [#754](https://github.com/aallan/vera/issues/754) |
