...
| Limitation | Issue |
|-----------|-------|
| An entry-file `data` declaration is not a party to per-owner ADT identity, so `E623` is not monotonic under adding a module. An entry declaring `Shape` beside ONE module declaring it with a different layout is refused; adding an unrelated SECOND module that also declares `Shape` makes the two modules contend, qualifies both to `mod$<path>$Shape` (spec §11.16), leaves the entry alone in the bare slot, and the refusal disappears. Both in-scope repairs were implemented and measured, and each breaks the other's rail: qualifying whenever the entry declares the name relaxes `E623` itself ([#1312](https://github.com/aallan/vera/issues/1312)'s rail), and declining the rename brings `E609` back and re-breaks [#1317](https://github.com/aallan/vera/issues/1317)'s local-declaration remedy. Needs a design ruling on how entry-level declarations interact with per-owner identity; both verdicts are pinned in `TestTheEntryRailIsNotMonotonicYet` so a resolution has to move them deliberately. | [#1423](https://github.com/aallan/vera/issues/1423) |
...
| A module cannot re-export an imported symbol, so deep module trees force consumers to import from the defining file. Sequenced in Milestone 4, behind the per-owner declaration identity that now gives a re-exported name somewhere unambiguous to land. | [#127](https://github.com/aallan/vera/issues/127) |
...
| Integer arithmetic wraps silently at a site where `_overflow_arith_codegen_type` declines — the checker's resolved-type table does not classify BOTH operands as `Int`/`Nat`, so no #798 overflow guard is emitted at all, while the `int_overflow` obligation is still recorded `tier3` as though one were.  Narrow: every site the classifier DOES cover traps, and the corpus has no instance.  This is the residual left after the `decreases`-measure half was closed — a `@Nat` measure's fit in the i64 its termination guard compares in is now obligated (`decreases_bound`) and either proved, runtime-checked or disclosed (`E536` / `E537`). | [#1417](https://github.com/aallan/vera/issues/1417) |
