...
| The `@Nat >= 0` narrowing **runtime** guard covers every concrete binding site — `let`, call argument, concrete constructor field, top-level match bind, ADT sub-pattern, tuple destructure — plus generic function-formal calls (guarded on the monomorphised callee). Three sites stay runtime-unguarded in an unverified compile: the effect-operation argument (`E.op(@Int.0)`), because codegen's `_effect_ops` carries only the dispatch target, not the op's formal types; the generic-instantiated **constructor field**, because constructor layouts carry no per-field `@Nat` metadata to monomorphise; and **builtin `@Nat` parameters** (`string_repeat`, `string_slice`, `string_pad_start`/`_end`, `md_has_heading`), whose special-cased translators bypass `_fn_nat_params` ([#757](https://github.com/aallan/vera/issues/757)). A tripped guard also surfaces a generic `unreachable` rather than the `requires(... >= 0)` fix. All three residual sites are statically obligated by the verifier, so verified programs are sound (#747); the effect-op guard + dedicated trap kind need only codegen plumbing (an op-parameter registry and the contract-fail host channel wired to a mid-expression guard). | [#754](https://github.com/aallan/vera/issues/754), [#757](https://github.com/aallan/vera/issues/757) |
