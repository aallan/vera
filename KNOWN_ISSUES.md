...
| Bug | Issue |
|-----|-------|
| `pip install -e .` fails on macOS arm64 with Apple Clang 15 because `z3-solver==4.16.0.0` has no prebuilt wheel for this platform, forcing a source build of Z3 itself — which then fails on Apple Clang 15's incomplete C++20 `<format>` support.  Affects the most common modern macOS development environment (Apple Silicon + default Xcode Clang); any agent or contributor following the documented install steps from SKILL.md hits this wall.  Workaround: install Xcode 16 / Clang 17, or use a Linux container, or pin to an earlier z3-solver release that has an arm64 wheel.  Candidate fixes (any one would help): pin `z3-solver` to a version with prebuilt arm64-macos wheels (e.g. `>=4.12,<4.15`); loosen the pin so pip picks a prebuilt wheel where available; document the Apple Clang 15 issue in SKILL.md; or allow linking against a system Z3 via `VERA_USE_SYSTEM_Z3=1`. | [#691](https://github.com/aallan/vera/issues/691) |
| `tests/test_browser.py::test_stdout_parity[hello_world]` intermittently times out on `test (windows-latest, 3.12)` after 30 seconds.  Asymmetric across the matrix (3.11 and 3.13 pass on the same runner) so this is Windows-runner Node-startup variance + `--experimental-wasm-exnref` cold V8 codegen, not a Vera regression.  Most recently seen on PR #693 ([CI run 26279976744](https://github.com/aallan/vera/actions/runs/26279976744)) — passed on retry of the same SHA.  Fix is a one-line bump of the two `timeout=30` literals in `tests/test_browser.py` (lines 160 and 1736) to `timeout=60`. | [#694](https://github.com/aallan/vera/issues/694) |
