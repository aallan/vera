...
| `_type_expr_to_wasm_type` in `vera/codegen/core.py` doesn't substitute parameterised aliases — `type Id<T> = T; @Id<Array<Int>>` returns `"unsupported"` because the alias body `NamedType("T")` is checked verbatim without binding `T` to `Array<Int>`.  Parallel to the gap closed by PR #631 in `_canonical_named_type` (the inference walker), but the compilability check goes through a different code path during function-decl registration.  `Box<T> = Array<T>` happens to work today via a coincidental bare-name match on `Array` in the alias body; `Id<T> = T` is the canonical broken case.  Surfaced during PR #631's review. | [#635](https://github.com/aallan/vera/issues/635) |
