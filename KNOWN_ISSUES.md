...
| The shared *leaf* discovery walk (`Monomorphizer.collect_calls_in_expr`) is not total over `Expr`: a generic reachable only from `ArrayLit` / `IndexExpr` / `InterpolatedString` is missed by both codegen and the #732 verifier, so it degrades to the Tier-3 `E520` residual (verify) or a compile error (codegen) rather than being verified. (`where`-helper bodies *are* walked on the verify side — #864 — so a where-fn-reachable generic is verified; codegen's discovery still misses them, a pre-existing compile-error limitation.) Safe today — both sides miss the leaf forms identically, so such a program is a compile error, not a runtime instance the verifier missed — but a latent false-Tier-1 footgun if codegen later emits a generic call from one of those forms. | [#768](https://github.com/aallan/vera/issues/768) |
