...
| Bug | Issue |
|-----|-------|
| `pip install -e .` fails on macOS arm64 with Apple Clang 15 because `z3-solver==4.16.0.0`'s macOS arm64 wheel (`z3_solver-4.16.0.0-py3-none-macosx_15_0_arm64.whl`) is tagged for macOS 15.0+ only — pip on macOS 13–14 doesn't match it and falls back to a source build of Z3, which then fails on Apple Clang 15's incomplete C++20 `<format>` support.  Affects the most common modern macOS development environment (Apple Silicon + default Xcode Clang on macOS 13/14); any agent or contributor following the documented install steps from SKILL.md hits this wall.  Workaround: upgrade to macOS 15+ (then pip matches the prebuilt wheel directly), install Xcode 17 / Clang 17 (then the source build succeeds), use a Linux container, or pin to an earlier z3-solver release whose arm64 wheel has a lower macOS minimum.  Candidate fixes (any one would help): pin `z3-solver` to a version with prebuilt arm64-macos wheels at a lower macOS minimum (e.g. `>=4.12,<4.15`); loosen the pin so pip picks a prebuilt wheel where available; document the Apple Clang 15 / macOS < 15 combination in SKILL.md; or allow linking against a system Z3 via `VERA_USE_SYSTEM_Z3=1`. | [#691](https://github.com/aallan/vera/issues/691) |
| `tests/test_browser.py::test_stdout_parity[hello_world]` intermittently times out on `test (windows-latest, 3.12)` after 30 seconds.  Asymmetric across the matrix (3.11 and 3.13 pass on the same runner) so this is Windows-runner Node-startup variance + `--experimental-wasm-exnref` cold V8 codegen, not a Vera regression.  Most recently seen on PR #693 ([CI run 26279976744](https://github.com/aallan/vera/actions/runs/26279976744)) — passed on retry of the same SHA.  Fix is a two-line bump — change both `timeout=30` literals in `tests/test_browser.py` (lines 160 and 1736) to `timeout=60`. | [#694](https://github.com/aallan/vera/issues/694) |
