...
| Bug | Issue |
|-----|-------|
| `Set<T>` host-store handles accumulate monotonically in `_set_store` across an `execute()` call. Same shape as the Map leak closed by [#573](https://github.com/aallan/vera/issues/573) phase 1; the fix is the same heap-wrap-as-ADT mirror applied to `set_new` / `set_add` / `set_remove` call-sites. Bounded by Python GC at `execute()` exit, so single-shot programs are unaffected. | [#575](https://github.com/aallan/vera/issues/575) |
| `Decimal` host-store handles accumulate monotonically in `_decimal_store` across an `execute()` call. Same shape as the Map leak closed by [#573](https://github.com/aallan/vera/issues/573) phase 1; the fix wraps every arithmetic op's output in a GC-managed wrapper ADT. Bounded by Python GC at `execute()` exit. | [#576](https://github.com/aallan/vera/issues/576) |
