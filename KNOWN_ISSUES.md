...
| Refinements over a **non-primitive base** (e.g. `{ @Array<Int> \| array_length(...) > 0 }`) are not yet runtime-guarded. Primitive-base refinements (`@Int`/`@Nat`/`@Bool`/`@Float64`/`@String`) are checked both statically (#746 — E505 on a provable violation) and at runtime (a predicate guard at the function boundary traps an unverified compile's violating value); a collection base, however, stays Tier 3 statically (the predicate translator does not lower a collection binder) and is not refinement-runtime-guarded — the underlying operations' own checks (e.g. array bounds) are the safety net. No statically-verified program is weakened. | [#762](https://github.com/aallan/vera/issues/762) |
| Generic (`forall<T>`) functions skip the E502 underflow obligation at verify time, because the verifier's early return for generics skips all static checks — this also means a concrete refined return on a generic function (`forall<T> fn f(@T -> @PosInt)`) is not *statically* obligated (#746 sub-case), though the codegen refinement guard still fires on its monomorphised instance so an unverified compile traps. Per-monomorphization static verification (#732) closes the static half. | [#555](https://github.com/aallan/vera/issues/555) |
