...
| The generic-instantiation discovery walk (shared by codegen and the #732 verifier) is not total over `Expr`: a generic reachable only from an un-walked form (`ArrayLit`, `IndexExpr`, `InterpolatedString`, a `where`-helper body) is missed by both, so it degrades to the Tier-3 `E520` residual rather than being verified. Safe today — codegen misses it identically, making such a program a compile error, not a runtime instance the verifier missed — but a latent false-Tier-1 footgun if codegen later emits a generic call from one of those forms. | [#768](https://github.com/aallan/vera/issues/768) |
