...
| The `@Nat >= 0` binding-site obligation does not yet cover ADT sub-pattern binds (`match opt { Some(@Nat.0) -> }` on `Option<Int>`) or non-literal tuple destructures, where the projected source type is not statically resolved. `let`, call-argument, constructor-field, match-bind, and literal-destructure narrowing are obligated (#552); closing the projection residual needs scrutinee-type inference. | [#747](https://github.com/aallan/vera/issues/747) |
