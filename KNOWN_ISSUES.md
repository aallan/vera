...
| The `@Nat >= 0` binding-site obligation does not yet cover ADT sub-pattern binds (`match opt { Some(@Nat.0) -> }` on `Option<Int>`) or non-literal tuple destructures (the projected source type is not statically resolved), and its Tier-3 runtime guard backs only the `let` site — so a narrowing whose value is untranslatable to Z3 at a call-argument, constructor-field, match-bind, or destructure site is caught by `vera verify` but not when compiled without it. Translatable narrowing at `let` / call-argument / constructor-field / match-bind / literal-destructure is statically obligated (#552); closing the rest needs scrutinee-type inference and per-site codegen guards. | [#747](https://github.com/aallan/vera/issues/747) |
