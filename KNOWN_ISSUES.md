...
| `IO.print` output is lost when the program traps. Host-side stdout is fully buffered (Rust's default when stdout isn't a TTY), so any `IO.print` calls preceding a runtime trap are discarded before reaching the terminal. Agents inserting instrumentation prints to debug a suspected crash see no output and conclude their changes weren't exercised. Fix: line-buffer stdout (flush on `\n`) in the host `IO.print` implementation, and/or add an explicit `IO.flush()` operation | [#522](https://github.com/aallan/vera/issues/522) |
