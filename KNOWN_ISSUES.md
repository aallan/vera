...
| Canonical Vera-type-name canonicalisation drift across `vera/wasm/inference.py`.  Ten distinct triggers of the same `i32_pair`-into-`i64` mismatch shape (silent fallthrough to `to_string(...)` over an `i32_pair` value, → `expected i64, found i32` at WASM validation) accumulated across #602, #614, PR #627, and PR #629.  Each trigger has been fixed locally; the underlying pattern remains exposed: five overlapping canonicalisation helpers (`_format_named_type`, `_format_named_type_canonical`, `_resolve_i32_pair_ret_te`, `_resolve_base_type_name`, ad-hoc `while`-loops) re-implement subsets of the same walk at different inference sites, and the silent fallthrough at `vera/wasm/operators.py:482` amplifies any one inference miss into invalid WASM rather than a compile error.  All 10 known triggers are pinned by regression tests (the 9th and 10th — inline `AnonFn` to `apply_fn` on both the Vera-type-name and WASM-type halves of the dispatcher, the latter with a `# pragma: no cover` claim that was empirically disproved — were both found by CodeRabbit within hours of filing this issue, providing decisive empirical confirmation that local-fix throughput can't keep up with discovery velocity); the 11th is a matter of when, not if.  Structural fix has two tiers: (T1) fold the helpers into a single `_canonical_vera_type` covering all four concerns; (T2) convert the silent fallthrough to a hard compile-time error.  Sibling of #626 — same silent-skip umbrella, single canonicalisation site rather than the broader translator-returns-None pattern. | [#630](https://github.com/aallan/vera/issues/630) |
