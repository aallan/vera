...
| The `@Nat >= 0` binding-site obligation does not yet cover ADT sub-pattern binds (`match opt { Some(@Nat.0) -> }` on `Option<Int>`), non-literal tuple destructures, or generic constructor fields instantiated to `@Nat` (`Some(@Int.0)` building an `Option<Nat>`) — each needs the source or call-site instantiation type, which the verifier does not statically resolve — and its Tier-3 runtime guard backs only the `let` site, so an untranslatable-value narrowing at a call-argument / constructor-field / match-bind / destructure site is caught by `vera verify` but not an unverified compile. Translatable narrowing at a concrete `let` / call-argument / constructor-field / match-bind / literal-destructure is statically obligated (#552); closing the rest needs call-site type inference and per-site codegen guards. | [#747](https://github.com/aallan/vera/issues/747) |
