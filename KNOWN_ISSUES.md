...
| The `@Nat >= 0` narrowing **runtime** guard covers every concrete binding site — `let`, call argument, concrete constructor field, top-level match bind, ADT sub-pattern, tuple destructure — plus generic function-formal calls (guarded on the monomorphised callee) and the builtin `@Nat` parameters that use the value unguarded (`string_repeat`, `string_pad_start`/`_end`, `string_from_char_code`, `md_has_heading`; `string_slice` already clamps its indices to `[0, len]`, so it needs no trap). Two sites stay runtime-unguarded in an unverified compile: the effect-operation argument (`E.op(@Int.0)`), because codegen's `_effect_ops` carries only the dispatch target, not the op's formal types; and the generic-instantiated **constructor field**, because constructor layouts carry no per-field `@Nat` metadata to monomorphise. A tripped guard also surfaces a generic `unreachable` rather than the `requires(... >= 0)` fix. Both residual sites are statically obligated by the verifier, so verified programs are sound (#747); the effect-op guard + dedicated trap kind need only codegen plumbing (an op-parameter registry and the contract-fail host channel wired to a mid-expression guard). | [#754](https://github.com/aallan/vera/issues/754), [#757](https://github.com/aallan/vera/issues/757) |
