...
| Type aliases over `Array<T>` break WASM codegen — type-checker resolves the alias and accepts the program, but codegen doesn't propagate the alias-to-target mapping. Two failure modes: alias as a parameter type emits structurally invalid WAT (`type mismatch: expected a type but nothing on stack` at instantiate); alias in a `let` binding triggers a silent `E602` skip of the enclosing function. Workaround: use the underlying type directly (`Array<Bool>` not `type Row = Array<Bool>`). Single-pointer ADT aliases (`Option<Int>`, user `data` types) are believed unaffected. | [#583](https://github.com/aallan/vera/issues/583) |
| User-defined `@Unit`-returning function call in non-tail statement position emits invalid WAT — block-statement sequencing in `vera/wasm/context.py::_is_void_expr` recognises `IO.*` qualified calls, `UnitLit`, effect-op `FnCall`s, and compound expressions as void, but doesn't recognise `FnCall` to a user function declared with `@Unit` return.  Falls through to "this expression produces a value", emits a `drop`, and WASM validation fails because the call left nothing on the stack.  The natural `render(grid); IO.sleep(120); recurse(...)` shape lands directly on this whenever `render` is a user helper.  Workaround: keep user `@Unit`-returning calls in tail position (fold trailing actions into the helper, or reshape the loop so the recursive call is the tail expression). | [#584](https://github.com/aallan/vera/issues/584) |
