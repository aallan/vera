...
| Runtime traps lack per-class `Fix:` suggestion paragraphs. Stage 1 (kind taxonomy: `divide_by_zero`/`out_of_bounds`/`stack_exhausted`/`unreachable`/`overflow`/`contract_violation`/`unknown` carried by `WasmTrapError.kind` and the JSON envelope's `trap_kind` field) shipped in v0.0.120; Stage 2 (per-function source backtrace via `WasmTrapError.frames` and the JSON envelope's `frames` array — `[{func, file, line_start, line_end, is_builtin}, …]`, leaf-first) shipped in v0.0.124. Stage 3 (per-kind `Fix:` paragraphs matching the rest of the toolchain's diagnostic style) remains open | [#516](https://github.com/aallan/vera/issues/516) |
