...
| CI test matrix omits `windows-latest`.  Vera's matrix today is `{ubuntu, macos} × {3.11, 3.12, 3.13}` = 6 entries; OS-specific bugs (path separators, line endings, locale, wasmtime / Playwright Windows quirks, subprocess shell dispatch) wouldn't surface until shipped to a Windows user.  PR #631's review cycle highlighted the gap — the existing matrix caught a Python-version-specific bug (PEP 701 multi-line f-string at 3.11) but a hypothetical Windows-only-at-3.11 bug would have shipped.  Proposal: add three `windows-latest` entries mirroring per-version coverage (9 total).  Cost: ~3-9 min per push parallel CI minutes; reasonable at current commit cadence. | [#637](https://github.com/aallan/vera/issues/637) |
