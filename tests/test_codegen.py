...
"""
    ...
    def test_e615_in_closure_body_emits_diagnostic(self) -> None:
        ...
        Note: this test pins the *diagnostic* surface only.  The
        closure-body failure does **not** currently propagate to
        drop the enclosing top-level function (`main` here) — its
        WAT was already generated by the time the closure-compile
        worklist runs, so the module is emitted with a
        `call_indirect` to a missing function-table entry and
        fails at WASM validation/instantiation.  Pinning that
        surface explicitly: the E615 emits, but `main` *remains*
        in `result.exports`, and execution traps.  Closing the
        enclosing-fn-drop is tracked as a follow-up; here we just
        ensure the diagnostic side of the post-#630 invariant
        holds.
        """
        source = _IO_PRELUDE + """\
public fn main(-> @Unit)
...
"""
        ...
        e615 = [d for d in warnings if d.error_code == "E615"]
        # The post-#630 closure-body harvest must emit at least
        # one [E615].  Pre-fix the closure was silently dropped
        # with no [E615] anywhere.
        assert e615, (
            f"Expected at least one [E615] from the closure body; "
            f"warnings were: {warnings}"
        )
        # Document the current "diagnostic emits but enclosing fn
        # isn't dropped" surface so a future enclosing-fn-drop
        # implementation will need to update this assertion (and
        # the docstring).  `main` *remains* in exports today
        # because main's WAT is already generated by the time the
        # closure-compile worklist returns None.
        assert "main" in result.exports, (
            f"Today main IS still exported despite the closure "
            f"failure — when this assertion starts failing, the "
            f"enclosing-fn-drop has been implemented; tighten the "
            f"test accordingly.  Current exports: {result.exports}"
        )
