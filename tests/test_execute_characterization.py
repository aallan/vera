...
decomposition target in the codebase: a ~3,400-line body with 154
...
already pinned by the GC-reclamation suites (``test_stress.py``).
...
confirms each test flips RED when its target return path is deliberately
broken, so none is green-for-the-wrong-reason.
"""
...
# lowers an OOB index to a WASM ``unreachable`` (test_codegen.py:4620);
# the index is a param so #680 routes it to honest Tier-3 at verify, and
# execute() (no verify) just traps at runtime.  The array is let-bound
# before indexing (the proven shape at test_codegen.py:4614) — a bare
# index on an array *literal* does not register an export.
_ARRAY_INDEX_OOB = """\
