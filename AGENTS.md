...
The conformance suite in `tests/conformance/` contains 75 small, self-contained programs — one per language feature — that serve as minimal working examples. Each program must pass its declared verification level (see `manifest.json` for mappings: `parse`, `check`, `verify`, or `run`). When you need to see how a specific construct works (e.g. effect handlers, match expressions, closures), check the corresponding conformance program before reading the spec.
...
python scripts/check_conformance.py    # All 75 conformance programs must pass
...
- All 75 conformance programs in `tests/conformance/` must pass their declared level
