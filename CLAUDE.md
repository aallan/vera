...
python scripts/check_examples.py      # Verify all 16 examples parse + check + verify
...
- `examples/` — 16 example Vera programs (all must pass `vera check` and `vera verify`)
- `tests/` — Test suite
...
- All 16 examples in `examples/` must pass `vera check` and `vera verify`
