...
python scripts/check_conformance.py    # Verify all 253 conformance programs (positives pass their level; negatives fail with their expected_error E-code)
...
- `tests/conformance/` — 253 conformance programs validating every language feature against the spec
...
- Every Vera block in the agent-facing docs must pass `scripts/check_doc_examples.py`: it parses, checks, verifies and prints what each `vera:run` marker says, or carries a `vera:skip-<stage>` marker with a category and a reason (`scripts/doc_annotations.py`)
