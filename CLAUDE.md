...
python scripts/check_conformance.py    # Verify all 255 conformance programs (positives pass their level; negatives fail with their expected_error E-code)
...
python scripts/check_corpus_canonical.py # Verify all 305 corpus programs are in canonical form (vera fmt)
...
python scripts/check_doc_counts.py --release # Also check the headline test totals against the live collection (the release PR sets them; CI runs this mode on PRs into main and pushes to main)
...
- `tests/conformance/` — 255 conformance programs validating every language feature against the spec
