...
python scripts/check_conformance.py    # Verify all 63 conformance programs pass their declared level
python scripts/check_examples.py      # Verify all 29 examples parse + check + verify
...
- `examples/` — 29 example Vera programs (all must pass `vera check` and `vera verify`)
- `tests/` — Test suite (unit tests + conformance suite)
- `tests/conformance/` — 63 conformance programs validating every language feature against the spec
...
- All 63 conformance programs in `tests/conformance/` must pass their declared level
- All 29 examples in `examples/` must pass `vera check` and `vera verify`
