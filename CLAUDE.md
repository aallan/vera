...
python scripts/check_conformance.py    # Verify all 62 conformance programs pass their declared level
python scripts/check_examples.py      # Verify all 28 examples parse + check + verify
...
- `examples/` — 28 example Vera programs (all must pass `vera check` and `vera verify`)
- `tests/` — Test suite (unit tests + conformance suite)
- `tests/conformance/` — 62 conformance programs validating every language feature against the spec
...
- All 62 conformance programs in `tests/conformance/` must pass their declared level
- All 28 examples in `examples/` must pass `vera check` and `vera verify`
