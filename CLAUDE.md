...
python scripts/check_conformance.py    # Verify all 78 conformance programs pass their declared level
python scripts/check_examples.py      # Verify all 31 examples parse + check + verify
...
- `examples/` — 31 example Vera programs (all must pass `vera check` and `vera verify`)
- `tests/` — Test suite (unit tests + conformance suite)
- `tests/conformance/` — 78 conformance programs validating every language feature against the spec
