...
| Nested generic helper in an **imported** fn | An imported non-generic function whose `where`-helper is `forall<T>` fails `vera compile` with an internal unknown-func error — the module body's call to its own nested generic is never discovered (the import-door variant of the fixed #990). | Hoist the module's generic helper to a top-level `public`/`private fn` in the module (top-level module generics monomorphize through the door). | [#999](https://github.com/aallan/vera/issues/999) |
