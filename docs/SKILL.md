...
|---|---|---|---|
| `handle[State<T>]` custom clauses | Clause bodies and `with` state-update expressions are type-checked but not executed — the runtime always applies builtin cell semantics (`get` returns the cell, `put` stores its argument). | Write `State` clauses only in the canonical form (`get(@Unit) -> { resume(@T.0) }`, `put(@T) -> { resume(()) } with @T = @T.0`); do not rely on custom handler logic for `State`. | [#976](https://github.com/aallan/vera/issues/976) |
| Nested `None` under `forall` | `Some(None)` returned as `@Option<Option<T>>` from a `forall<T>` function is rejected with E121 (inner constructor's type variable is not unified with the declared var; the non-nested case works). | Bind the inner value first (`let @Option<T> = None`) and wrap the binding, or construct at a concrete type. | [#979](https://github.com/aallan/vera/issues/979) |
| Generic `where`-helper under a non-generic parent | A `forall<T>` helper inside a non-generic function's `where` block is never monomorphized — `vera check`/`verify` pass but `vera compile` fails with an internal unknown-func error. | Hoist the generic helper to a top-level `private fn` (top-level generics monomorphize correctly), or make the parent `forall<T>` too. | [#990](https://github.com/aallan/vera/issues/990) |
| Duplicate `where`-helper names | Two helpers sharing a name (sibling subtrees, or a helper named like a top-level fn) crash `vera compile` with a raw `duplicate func identifier` WAT error on a check-green program. | Give every helper in a function's `where` tree a unique name, distinct from all top-level function names. | [#991](https://github.com/aallan/vera/issues/991) |
| `eq`/`compare` in an imported function | Any imported function whose body (or helper) calls `eq(...)`/`compare(...)` fails `vera compile` with an internal unknown-func error — the ability-op rewrite never runs over imported bodies. | Replace `eq(a, b)` with `==` (lowers directly). `compare(...)` has no drop-in operator — it returns `Ordering` — so imported code that matches on `Less`/`Equal`/`Greater` must build the `Ordering` another way (an explicit `<`/`==` if-chain) or stay out of imported modules until this is fixed. | [#992](https://github.com/aallan/vera/issues/992) |
| `None` in contract comparisons | A bare `None` as an operand of `==` in a contract under `forall<T>` (e.g. `ensures(@Option<T>.result == None)`) is rejected E142 — the comparison types each operand with no expected type, so `None` mints an unrelated fresh var (`Cannot compare Option<T> with Option<T$1>`). | State the postcondition with `match` instead of `==`: `ensures(match @Option<T>.result { None -> true, Some(@T) -> false })`. | [#981](https://github.com/aallan/vera/issues/981) |
| `@Int` → `@Nat` narrowing in a closure return | A closure `fn(@Int -> @Nat) { @Int.0 }` applied to a negative value returns it silently — the closure return leaf is neither obligated nor guarded (top-level `fn` and `where`-helper returns ARE, via #758). | Narrow through a named `where`-helper (`fn(@Int -> @Nat) { to_nat(@Int.0) }`) or a `let @Nat` binding inside the closure (`{ let @Nat = @Int.0; @Nat.0 }`) — both trap on a negative. | [#984](https://github.com/aallan/vera/issues/984) |
| `@Nat` → `@Int` widening in an **imported** function | The #820 array-element and tuple-component widen guards are not emitted for imported module bodies (the target-type table is single-module): the library's own `vera verify` reports the widening Tier-3 runtime-guarded, but the importing program's compiled artifact drops the guard and a `@Nat` above `2^63 - 1` reinterprets to a negative `@Int` silently. | Constrain the widening in the library with `requires(... <= 9223372036854775807)` (Tier-1-provable, no runtime guard needed), or bind through `let @Int` before the array/tuple component (the `let`-site guard survives cross-module). | [#987](https://github.com/aallan/vera/issues/987) |
