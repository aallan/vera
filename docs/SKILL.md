...
| Nested `where` blocks | A helper that carries its own `where`-helpers checks green but fails `vera compile`: the non-generic codegen path never emits the grandchild helpers, leaving a dangling unknown-func reference. | Flatten helpers to a single `where`-block level (siblings under one parent), or make the parent generic — both emit their helpers correctly. | [#978](https://github.com/aallan/vera/issues/978) |
