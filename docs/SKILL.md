...
| `eq`/`compare` in an imported function | Any imported function whose body (or helper) calls `eq(...)`/`compare(...)` fails `vera compile` with an internal unknown-func error — the ability-op rewrite never runs over imported bodies. | Use the native operators (`==`, `<`, `>`) in library code instead of the `eq`/`compare` ability calls — they lower directly and are unaffected. | [#992](https://github.com/aallan/vera/issues/992) |
