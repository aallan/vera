...
The `tests/conformance/` directory contains 90 small, self-contained programs that validate every language feature against the spec — often one program per feature, though some features (slot references, match, contracts) span several. These are the best minimal working examples of Vera syntax and semantics.
...
|-----------|---------|-------|
| Refinement predicates verified statically, not runtime-guarded | A refinement type `{ @T \| P }` has its predicate discharged **statically** at every narrowing site (`let`, call argument, constructor field, effect-op argument, match binding, tuple component) and at refined return positions — a violation is an `E505` error, an untranslatable case an `E506` Tier-3 warning. The compiler does not yet emit a *runtime* guard for a predicate it cannot prove statically (non-primitive base, undecidable construct, timeout), so such a Tier-3 narrowing — including at a `public`/FFI boundary — is unchecked at run time. Write a `requires(...)` implying the predicate, or keep refined values flowing only from already-refined sources. | [#762](https://github.com/aallan/vera/issues/762) |
