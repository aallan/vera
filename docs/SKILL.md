...
| Widening in an imported **generic** fn | A library `forall<T>` fn whose body widens `@Nat` into an `@Int` array/tuple component loses the runtime guard at every instantiation when imported (`u64.MAX` silently reinterprets), though the library's own `vera verify` reports Tier-3. | Constrain the widening in the library with `requires(... <= 9223372036854775807)` (Tier-1, no runtime guard needed), or make the widening fn non-generic (non-generic imported bodies are guarded since #987). | [#998](https://github.com/aallan/vera/issues/998) |
