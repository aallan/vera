...
**v0.0.101** delivers the `<Inference>` algebraic effect — the headline feature that most differentiates Vera from other verified languages. `Inference.complete(String → Result<String, String>)` dispatches to Anthropic, OpenAI, or Moonshot via env vars and is host-backed (Python `urllib.request` / rich browser error). LLM calls are now explicit in the type system, contract-verifiable, and impossible to call from pure functions. Closes [#61](https://github.com/aallan/vera/issues/61).

**v0.0.100** delivers a full compiler pipeline (parse → typecheck → verify → compile → run), 113 built-in functions plus 5 Option/Result combinators plus 4 higher-order array operations, a module system, algebraic effect handlers, constrained generics with four built-in abilities (Eq, Ord, Hash, Show), a 64-program conformance suite, a canonical formatter, and contract-driven testing. `Map<K, V>`, `Set<T>`, and `Decimal` provide collections and exact arithmetic with ability constraints. `Json` provides structured data interchange. `HtmlNode` provides HTML parsing and querying. The `<Http>` effect enables network I/O. The `<Inference>` effect enables LLM calls. A standard prelude eliminates boilerplate. An independent viability assessment rates Vera at **60–70% of the way to being a viable agent target** — and with Inference now shipping, the goal of an LLM writing verified, contract-checked LLM orchestration code is no longer theoretical.
...
- [#366](https://github.com/aallan/vera/issues/366) **JSON typed accessors** — `json_as_string`/`json_as_number`/`json_as_bool`/`json_as_int`/`json_as_array`/`json_as_object` (Json → Option<T>) and compound `json_get_string`/`json_get_number`/`json_get_bool`/`json_get_int`/`json_get_array` (Json, String → Option<T>). Eliminates the two-level pattern match every JSON API consumer currently writes. Pure Vera prelude functions, no new host imports.
- [#367](https://github.com/aallan/vera/issues/367) **Markdown content extractors** — `md_blocks`, `md_inline_text`, `md_block_text`, `md_extract_headings`, `md_extract_links`, `md_filter_blocks`. Completes the structural traversal story alongside the existing boolean-query and code-extraction functions. Pure Vera prelude, recursive over MdBlock/MdInline ADTs.
- [#368](https://github.com/aallan/vera/issues/368) **HTML convenience accessors** — `html_query_one` (first CSS match → Option<HtmlNode>), `html_tag` (element tag name), `html_children` (direct child nodes). Fills the gap between bulk CSS queries and single-element access. Pure Vera prelude or trivial host-import extensions.
- <del>[#61](https://github.com/aallan/vera/issues/61) **Inference effect** — `effects(<Inference>)` in a signature means "this function calls an LLM, and you can mock it for testing." The feature that most differentiates Vera from Dafny as a verification target, and the one that positions it as the natural language for verified LLM orchestration.</del> ([v0.0.101](https://github.com/aallan/vera/releases/tag/v0.0.101))
