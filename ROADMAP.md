...
Closing these — by building infrastructure that prevents recurrence ([#596](https://github.com/aallan/vera/issues/596) stress harness, [#597](https://github.com/aallan/vera/issues/597) walker audit) — converts "I think we're stabilising" into evidence.  After that, agent integration (LSP, `vera context`, Inference controls) becomes the priority.
...
The compiler was built through eleven stages from February 2026 onwards. **810+ commits, 138 tagged releases (as of v0.0.138), 3,760 tests, 96% coverage, 86 conformance programs, 33 examples, 13 spec chapters.** See [HISTORY.md](HISTORY.md) for the per-stage narrative and per-release table.
