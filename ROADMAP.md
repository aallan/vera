...
| Document tacit project conventions surfaced during v0.0.156 sweep | [#687](https://github.com/aallan/vera/issues/687) | 1–2 hours | CodeRabbit reply workflow nuances (push-before-reply, per-thread replies on declines, outdated auto-resolution), JSON output stability contract (e.g. `TestSummary.unlisted_errors`), doc-count gate awareness, modular-verification design-principle signposting.  Surfaced during release sweep as small individually-unimportant items that together close the friction loop for new contributors and AI agents. |
