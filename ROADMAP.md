...
| Issue | What |
|---|---|
| [#795](https://github.com/aallan/vera/issues/795) | **Extend mutation testing beyond the soundness core.**  Mutation testing deliberately breaks each line of `vera/` and confirms a test flips RED, catching *green-for-the-wrong-reason* tests (the #680 audit found 8 in one 57-test battery).  [#387](https://github.com/aallan/vera/issues/387) set up `mutmut` and hardened the **soundness core** (`verifier`/`smt`/`checker`/`obligations`) from the 80.8% baseline to **83.3% caught** ([#788](https://github.com/aallan/vera/pull/788) checker, [#793](https://github.com/aallan/vera/pull/793) verifier + smt; runbook `MUTATION.md`).  Remaining: (1) make the **full sweep reliable** — the 10,620-mutant marathon deadlocks (a `mutmut` 3.6 / Python 3.14 long-run worker hang; targeted per-function runs are fine), which blocks any large run; (2) the **whole-`vera/` sweep** over `codegen`/`wasm`/`transform`/`parser`/CLI, unblocked by the #421 decomposition (api.py's `execute()` no longer inflates a mutant file mutmut can't index).  The deep verifier / `smt.py` translate-layer hardening that needs the mutmut feedback loop is tracked in [#792](https://github.com/aallan/vera/issues/792).  The in-process oracle means subprocess-only suites (conformance / CLI / browser) can't kill mutants, so coverage comes from the in-process unit suites — *not* the conformance suite. |
