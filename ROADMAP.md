...
| Order | Issue | Why now |
|:---:|---|---|
| 1 | [#475](https://github.com/aallan/vera/issues/475) — WASM call translator bug cleanup (PR 2 of 2 — seven Major findings) | The three Critical findings (`string_char_code` memory-read-OOB, `string_slice` no clamping, expression-bodied `Exn` handler result type) shipped in v0.0.128. Seven Major correctness fixes remain: `array_slice` same-shape narrowing as the now-fixed `string_slice`; `_map_wasm_tag` Map<K, Array<T>> lowering; `url_parse` / `url_join` round-trip flag preservation; `base64_decode` `=`-in-non-padding rejection; `parse_nat` / `parse_int` embedded-space rejection; `to_string(INT64_MIN)` negation overflow; `float_to_string` rounded-fractional carry. Small, independent fixes; ship together as the Major-batch PR. |
| 2 | [#535](https://github.com/aallan/vera/issues/535) — Pair-type closure capture (residual of #514) | The original #514 framing ("all heap captures broken") was inaccurate: ADT captures work, only `String` and `Array<T>` (pair-typed values) silently lose their len field during closure-struct serialisation. The infrastructure for pair handling already exists — parameters use it via the `i32_pair` branch in `_compile_lifted_closure`; only the capture path needs to plug in. Workaround (lift to a top-level helper that takes the value as a parameter) keeps the pair-capture case usable until this lands. Estimated 1–2 hours given the parameter path already does the right thing. |
| 3 | [#487](https://github.com/aallan/vera/issues/487) + [#348](https://github.com/aallan/vera/issues/348) — GC infrastructure: allocator growth + worklist sizing | Two related memory-infrastructure bugs. #487: `$alloc` grows memory by only 1 page (64 KB), so single requests larger than 64 KB more than free heap space trap. #348: GC worklist has a fixed 16 KiB / 4,096-entry cap (`gc_worklist_size = 16384` in `vera/codegen/assembly.py`); a reachable object graph wider than that loses marks and gets swept as garbage. Both live in `vera/codegen/assembly.py`; fixable together. |
| 4 | [#346](https://github.com/aallan/vera/issues/346) + [#347](https://github.com/aallan/vera/issues/347) + [#490](https://github.com/aallan/vera/issues/490) — Opaque-handle hygiene | All three touch the handle-vs-pointer discrimination in the GC. #346: Map/Set/Decimal host stores are append-only so transient opaque handles leak Python-side. #347: opaque handle parameters get pushed to the GC shadow stack despite being i32 handles, not pointers. #490: `array_fold`'s ADT-rooting heuristic treats every non-Bool/Byte i32 as a heap pointer, over-rooting host-managed handles. Fix is a shared "is-this-actually-a-Vera-heap-pointer" classifier. |
...
**810+ commits, 128 tagged releases (as of v0.0.128), 3,658 tests, 96% coverage, 82 conformance programs, 33 examples, 13 spec chapters.** See [HISTORY.md](HISTORY.md) for the full narrative of how the compiler was built.
