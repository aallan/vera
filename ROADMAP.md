...
- [#618](https://github.com/aallan/vera/issues/618) **`IO.read_char` (and `Terminal` effect)** — single-character / raw-mode terminal input.  Tetris-class real-time programs are blocked today because `IO.read_line` is line-buffered.  `IO.read_char` alone is the cheap minimum; a full `Terminal` effect (`set_raw_mode`, `enable_alternate_screen`, `get_size`) unlocks every interactive demo.  Pairs with [#609](https://github.com/aallan/vera/issues/609) (browser JSPI sleep) and [#610](https://github.com/aallan/vera/issues/610) (browser ANSI subset interpreter) — together those three make terminal-style real-time programs portable.
...
- [#621](https://github.com/aallan/vera/issues/621) **`debug` effect for diagnostic stderr output in any context (including `pure`)** — single privileged escape hatch for printing-while-debugging.  Effect-system invisible (no signature change), verifier-invisible (treated as no-op), strippable in production builds.  Closures inside `array_map` / `array_all` are exactly where one wants `IO.print` and exactly where it's forbidden today.  Prior art: Koka's `unsafe-total`, Haskell's `Debug.Trace.trace`. |
- [#622](https://github.com/aallan/vera/issues/622) **SKILL.md polish: `array_join` cross-reference + `decreases`-with-+1 idiom example** — small documentation improvements surfaced by the Tetris experiment: cross-reference `string_join` from the array section (so agents reaching for `array_join` find it), and an example for the "row index counts down to -1, metric is `@Int.0 + 1`" idiom that's common but not currently documented.
...
- [#616](https://github.com/aallan/vera/issues/616) **Built-in `nat_add` / `nat_sub_sat` / `nat_mul`** — eliminates the five-line `int_to_nat` round-trip with unreachable `None` arm that's currently the canonical way to add two `Nat`s.  Most painful gap by frequency surfaced by the Tetris experiment.  Hour or two of work; verifier discharges totality directly via the `Nat` refinement.
...
### Phase 4d: Language ergonomics

Strict-superset additions surfaced by real-program friction.  Each preserves the existing semantics for unchanged code; only the boilerplate floor changes.

- [#619](https://github.com/aallan/vera/issues/619) **Single-keyword `pure` / `io` shorthand for the vacuous contract preamble** — `fn helper(@Int -> @Int) pure { ... }` desugars to `requires(true) ensures(true) effects(pure)`.  Only for the all-vacuous case; real contracts use the long form.  Tetris had ~20 helpers with 60 lines of vacuous preamble.
- [#620](https://github.com/aallan/vera/issues/620) **Optional field names in `data` declarations** — `MkState(board: Array<Int>, x: Int, y: Int, ...)` for `--explain-slots` and error-message use only.  Slot semantics unchanged; match arms still index by position.  Helps agents distinguish `@Int.0` (rightmost / `y`) from `@Int.1` (`x`) without a reminder comment under every match arm.
...
| Anonymous tuples / pairs with destructuring (`let (x, y) = ...`) | [#617](https://github.com/aallan/vera/issues/617) | A real Vera program would benefit from returning a pair without either an `Array<Int>` index-arithmetic dance or a single-variant `data Pair { Pair(...) }` ADT.  Today: those two workarounds are the canonical shape; the friction document explicitly de-prioritises tuples below the rest of the friction list (*"probably a bigger language change than it looks. The slot system has clean answers for ADTs and arrays; tuples would need their own slot-stack story or would overlap awkwardly with single-variant ADTs"*).  Trigger: enough real programs hit the indexing-arithmetic-in-closure trap-class (#615) to make tuples the cheaper fix, OR a stdlib API needs to return a pair and the workarounds become awkward. |
