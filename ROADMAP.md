...
- [#525](https://github.com/aallan/vera/issues/525) **Close remaining [Agent Score](https://buildwithfern.com/agent-score) gaps on veralang.dev** — three checks still fail after the v0.0.119 homepage redesign: `content-negotiation` (GitHub Pages can't honour `Accept: text/markdown` natively — needs a Cloudflare Worker edge rule or a move to a host that supports `_redirects`); `content-start-position` (inline `<style>` block consumes agent truncation budget; small win available by moving the JSON-LD `<script>` to the end of `<body>`, larger fix requires relaxing the single-file HTML constraint); `markdown-content-parity` improved 90% → 21% missing but still fails — remaining gap is interface chrome (CTA labels, readpath device, eyebrows) that doesn't translate naturally to markdown.
