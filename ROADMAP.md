...
A second Game of Life agent run against v0.0.119 surfaced five fresh compiler/runtime bugs in a single afternoon ([#514](https://github.com/aallan/vera/issues/514), [#515](https://github.com/aallan/vera/issues/515), [#516](https://github.com/aallan/vera/issues/516), [#517](https://github.com/aallan/vera/issues/517), [#522](https://github.com/aallan/vera/issues/522)), plus [#520](https://github.com/aallan/vera/issues/520) from targeted testing. Combined with the pre-existing GC and translator bugs ([#346](https://github.com/aallan/vera/issues/346), [#347](https://github.com/aallan/vera/issues/347), [#348](https://github.com/aallan/vera/issues/348), [#475](https://github.com/aallan/vera/issues/475), [#487](https://github.com/aallan/vera/issues/487), [#490](https://github.com/aallan/vera/issues/490)), that started at twelve open bug issues. v0.0.120 closed the first pair (#522 fully, #516 Stage 1); v0.0.121 closed #514 (nested closures + ADT capture work end-to-end) and split the residual pair-type capture out as [#535](https://github.com/aallan/vera/issues/535); v0.0.122 closed #515 (`$gc_collect` now bounds the conservative scan against `$heap_ptr` at two layers); v0.0.124 closed #516 Stage 2 (per-function source backtrace) and v0.0.125 closed Stage 3 / [#547](https://github.com/aallan/vera/issues/547) (per-`kind` `Fix:` paragraphs); v0.0.126 closed #517 (WASM `return_call` for tail-position calls — non-allocating tail recursion now runs in constant stack space); v0.0.127 closed #520 (the `@Nat` subtraction soundness hole — verifier now emits a Tier-1 `lhs >= rhs` proof obligation at every `@Nat - @Nat` site that has `@Nat` provenance, and the codegen emits a runtime guard for the same set; pure-literal subtractions like `0 - 1` are intentionally exempt at Path-A scope and tracked by [#552](https://github.com/aallan/vera/issues/552) for the binding-site generalisation); 2026-05-04 housekeeping closed [#551](https://github.com/aallan/vera/issues/551) as not-a-bug (the `@Byte` arithmetic case it described is checker-prevented — the type checker excludes `Byte` from `NUMERIC_TYPES`, so no runtime hole exists; the speculative *feature* of allowing Byte arithmetic with verified guards is now tracked under [#564](https://github.com/aallan/vera/issues/564) in the Speculative section below); v0.0.128 shipped PR 1 of 2 for #475 (the three Critical safety findings — `string_char_code` bounds check, `string_slice` clamp-before-narrow, expression-bodied `Exn` handler result type — closing the memory-read-OOB hole and two correctness silently-wrong-output bugs); v0.0.129 closed #475 in full with PR 2 (the seven Major correctness findings — `array_slice` clamp, `Map<K, Array<T>>` codegen rejection, `url_parse` / `url_join` round-trip flag preservation, `base64_decode` `=`-position validation, `parse_nat` / `parse_int` embedded-space rejection, `to_string(INT64_MIN)` unsigned loop-break, and `float_to_string` carry-on-rounded-fraction); v0.0.130 closed #535 (pair-type closure captures — `_walk_free_vars` now tags `String` / `Array<T>` captures as `i32_pair` and the closure-struct emit + load code paths serialise both ptr and len, so `array_length` / `string_length` of a captured pair no longer silently reads as zero); v0.0.131 closed #487 + #348 in a single batch (`$alloc` now computes `pages_needed = ceil(shortage / 65536)` for `memory.grow` so single allocations of any practical size succeed; the GC mark-phase worklist is quadrupled to 64 KiB / 16 384 entries, and both push branches now `unreachable` on overflow rather than silently dropping — covered by `TestLargeAllocGrow487` and `TestWorklistOverflow348`); v0.0.132 closed #347 + #490 (the codegen-time half of the opaque-handle hygiene work — a new `_is_host_handle_type` classifier excludes `Map` / `Set` / `Decimal` from GC-rooting at every decision site so these i32 handles no longer pollute the shadow stack or get over-rooted in `array_fold`/`array_map` accumulators).  #346 (the active reclamation half — host-store leak across an `execute()` call) is deferred; an earlier draft of the v0.0.132 PR attempted to close all three with a `host_gc_sweep` mechanism, but the design grew six interlocking pieces and the practical impact (per-`execute()` leak, bounded by Python GC at exit) didn't justify the complexity.  The recommended follow-up design is heap-wrap-as-ADT (return a Vera-heap `MapHandle(i32)` from handle-creating ops so the existing mark-sweep GC handles reclamation), tracked separately as [#573](https://github.com/aallan/vera/issues/573).  Two remain in the campaign (#346, #570) plus one enhancement ([#507](https://github.com/aallan/vera/issues/507)).
...
| Order | Issue | Why now |
|:---:|---|---|
| 1 | [#346](https://github.com/aallan/vera/issues/346) → [#573](https://github.com/aallan/vera/issues/573) — Active reclamation of host-store handles | The codegen-time half of the opaque-handle work (#347 + #490) closed in v0.0.132.  This is the residual: Python-side stores (`_map_store` / `_set_store` / `_decimal_store` in `vera/codegen/api.py`) accumulate monotonically across an `execute()` call.  Tracked in detail at [#573](https://github.com/aallan/vera/issues/573); recommended design is heap-wrap-as-ADT (return a Vera-heap `MapHandle(i32)` from each handle-creating op so the existing mark-sweep GC handles reclamation via a destructor callback) which avoids the parallel-reclamation complexity that an earlier draft attempted with `host_gc_sweep`. |
| 2 | [#570](https://github.com/aallan/vera/issues/570) — `array_map` shadow-stack overflow at ~4 000 elements | The lifted closure's epilogue pushes the return-value root to the shadow stack AFTER the `gc_sp` restore, expecting the caller to root the in-flight ptr.  Iterative-builder callers (`array_map`'s loop body) store the return into `dst[idx]` immediately and never unwind the post-restore push — one slot leaks per iteration, so a 4 000-element `array_map` over an ADT-producing closure exhausts the 16 KiB / 4 096-entry shadow stack.  Cleanest fix is unwinding the closure-return root in the iterative-builder caller (or restructuring the closure epilogue to skip the push when the return is going to be stored immediately).  `array_filter` / `array_mapi` share the pattern; `array_flatten` has no callback path so isn't affected. |
| 3 | [#507](https://github.com/aallan/vera/issues/507) — Eq/Ord-dispatched array ops (`array_sort`, `array_contains`, `array_index_of`) | Phase 2 of [#466](https://github.com/aallan/vera/issues/466) (phase 1 mapi/reverse/find/any/all/flatten/sort_by shipped in v0.0.117). Needs the dispatch infrastructure to invoke `compare$T`/`eq$T` from inside an iterative WASM loop. Lower urgency than the other entries because explicit-callback alternatives (`array_sort_by`, `array_any` + equality predicate) already work. |
...
- [#539](https://github.com/aallan/vera/issues/539) **`vera builtins/effects/errors --json` — compiler introspection subcommands** — three thin CLI wrappers around the compiler's built-in / effect / error-code registries. Makes Vera the source of truth for "164 built-in functions" instead of CLAUDE.md being canon (and thus drifting silently when built-ins are added — repeatedly hit during the v0.0.121 review cycle). Same agent-ergonomics shape as `--json` diagnostics: structured output that LSPs / editors / agents can parse. Independent half-day units; ship one at a time. `vera errors --json` highest value (unblocks generated error-reference table in spec); `vera builtins --json` next (8+ docs reference the count); `vera effects --json` cheapest (small stable registry). Each item carries a `since` field for future "what's new in 0.x" generation.
...
| Add mutation testing with mutmut (detection only) | [#387](https://github.com/aallan/vera/issues/387) | 2–4 hours | Measures whether 3,658 tests catch real bugs, not just execute paths |
...
**810+ commits, 132 tagged releases (as of v0.0.132), 3,698 tests, 96% coverage, 82 conformance programs, 33 examples, 13 spec chapters.** See [HISTORY.md](HISTORY.md) for the full narrative of how the compiler was built.
