...
4,312 tests, 89 conformance programs, 35 examples, 13 spec chapters.
...
| Add `ubuntu-24.04-arm` to CI matrix for Linux aarch64 coverage | [#702](https://github.com/aallan/vera/issues/702) | 30–60 min | Closes the "CI is masked from a real platform" gap surfaced by #691: today CI only runs on `ubuntu-latest` (x86_64) so Linux aarch64 regressions are invisible until a user reports one.  GitHub now provides `ubuntu-24.04-arm` as a hosted runner on the free tier.  Adds one matrix entry (× Python 3.12 only — platform coverage, not Python coverage).  Complements `scripts/check_wheel_availability.py` which checks dep-resolution; this issue adds actual end-to-end test execution. |
