...
| Order | Issue | Why now |
|:---:|---|---|
| 1 | [#346](https://github.com/aallan/vera/issues/346) + [#347](https://github.com/aallan/vera/issues/347) + [#490](https://github.com/aallan/vera/issues/490) — Opaque-handle hygiene | All three touch the handle-vs-pointer discrimination in the GC. #346: Map/Set/Decimal host stores are append-only so transient opaque handles leak Python-side. #347: opaque handle parameters get pushed to the GC shadow stack despite being i32 handles, not pointers. #490: `array_fold`'s ADT-rooting heuristic treats every non-Bool/Byte i32 as a heap pointer, over-rooting host-managed handles. Fix is a shared "is-this-actually-a-Vera-heap-pointer" classifier. |
| 2 | [#570](https://github.com/aallan/vera/issues/570) — `array_map` shadow-stack overflow at ~4 000 elements | Each iteration of `array_map` over a heap-allocating closure body pushes the new element's ptr to the GC shadow stack but never unwinds within the loop, exhausting the 16 KiB / 4 096-entry stack. Surfaced while writing #348's runtime regression test. Cleanest fix is per-iteration shadow-stack unwind in the array_map codegen (or eliminating the per-element root by writing directly into the rooted destination array). Sibling builders (`array_filter`, `array_mapi`, `array_flatten`) likely share the pattern. |
...
**810+ commits, 131 tagged releases (as of v0.0.131), 3,694 tests, 96% coverage, 82 conformance programs, 33 examples, 13 spec chapters.** See [HISTORY.md](HISTORY.md) for the full narrative of how the compiler was built.
