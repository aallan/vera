...
Strategy mirrors check_readme_examples.py:
  1. Extract all fenced code blocks tagged as `vera` from EXAMPLES.md.
...
# -- Allowlist: EXAMPLES.md blocks that are intentionally unparseable. -----
#
...
def try_parse(content: str) -> str | None:
    ...
    try:
        parse(content, file="<examples-doc>")
        return None
...
def main() -> int:
    ...
    examples_doc = root / "EXAMPLES.md"

    if not examples_doc.is_file():
    ...
    # Report
    print(f"EXAMPLES.md code blocks: {total_blocks} total")
    ...
    if stale_allowlist:
        ...
        for line_no, category, reason in stale_allowlist:
            print(
                f"  EXAMPLES.md line {line_no} [{category}]: {reason}",
                file=sys.stderr,
            )
        print(
    ...
    if failures:
        ...
        for line_no, error in failures:
            print(f"\n  EXAMPLES.md line {line_no}:", file=sys.stderr)
            print(f"    {error}", file=sys.stderr)
        print(
            f"\n{len(failures)} EXAMPLES.md code block(s) failed to parse.",
            file=sys.stderr,
        )
        print(
        ...
        print(
            "in scripts/check_examples_doc.py with the appropriate category.",
            file=sys.stderr,
        )
        exit_code = 1

    if exit_code == 0:
        print("\nAll EXAMPLES.md Vera code blocks parse successfully.")
