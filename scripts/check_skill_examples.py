...
ALLOWLIST: dict[int, tuple[str, str]] = {
    ...
    468: ("FRAGMENT", "Option/Result combinator usage examples, bare calls"),

    # Types section — bare type expressions, not declarations
    310: ("FRAGMENT", "Composite type examples, bare expressions"),
    327: ("FRAGMENT", "Type alias examples"),

    # Control flow — bare expressions
    399: ("FRAGMENT", "If/else expression example"),
    413: ("FRAGMENT", "Block expression example"),

    # Array operations — bare function calls
    505: ("FRAGMENT", "Array built-in examples, bare calls"),

    # Map operations — bare function calls
    523: ("FRAGMENT", "Map built-in examples, bare calls"),

    # Handler syntax — pseudocode template
    1131: ("FRAGMENT", "Effect handler syntax template"),

    # Common mistake: bare if/else expression
    1538: ("FRAGMENT", "Common mistake example, bare if/else"),

    # String operations — bare function calls
    605: ("FRAGMENT", "String built-in examples, bare calls"),

    # HTML operations — bare function calls and match expression
    722: ("FRAGMENT", "HTML built-in examples, bare calls"),
    738: ("FRAGMENT", "HTML match expression example, bare expression"),

    # Set operations — bare function calls
    540: ("FRAGMENT", "Set built-in examples, bare calls"),

    # Decimal operations — bare function calls
    554: ("FRAGMENT", "Decimal built-in examples, bare calls"),

    # JSON operations — bare function calls
    577: ("FRAGMENT", "JSON built-in examples, bare calls"),
    590: ("FRAGMENT", "JSON match expression example, bare expression"),

    # Markdown operations — bare function calls
    686: ("FRAGMENT", "Markdown built-in examples, bare calls"),

    # Regex operations — bare function calls
    753: ("FRAGMENT", "Regex built-in examples, bare calls"),
    764: ("FRAGMENT", "Regex Result matching example, bare expression"),

    # String interpolation — bare expressions
    649: ("FRAGMENT", "String interpolation examples, bare expressions"),

    # String search — bare function calls
    661: ("FRAGMENT", "String search built-in examples, bare calls"),

    # String transformation — bare function calls
    672: ("FRAGMENT", "String transformation built-in examples, bare calls"),

    # Numeric operations — bare function calls
    776: ("FRAGMENT", "Numeric built-in examples, bare calls"),

    # Contracts section — requires/ensures fragments
    833: ("FRAGMENT", "Requires clause example, not full function"),
    842: ("FRAGMENT", "Ensures clause example, not full function"),

    # Quantified expressions — bare forall/exists calls
    869: ("FRAGMENT", "Quantified expression examples, bare calls"),
    ...
    1114: ("FRAGMENT", "Handler syntax template, not real code"),

    # Effect declarations — bare effects(...) clauses
    887: ("FRAGMENT", "Effect declarations list"),
    992: ("FRAGMENT", "Async effect declarations list"),
    1012: ("FRAGMENT", "Async effect declarations, bare clauses"),
    1042: ("FRAGMENT", "Http effect declarations list"),
    1073: ("FRAGMENT", "Inference effect declarations list"),

    # Qualified calls and handler fragments — bare expressions
    1143: ("FRAGMENT", "Handler with clause, bare expression"),

    # Module declaration and import syntax
    1191: ("FRAGMENT", "Module declaration and import example"),

    # Line comments — bare comments
    1296: ("FRAGMENT", "Comment syntax example"),

    # Type conversions — bare function calls
    804: ("FRAGMENT", "Float64 predicate and constant examples, bare calls"),

    # Float64 predicates — bare function calls
    791: ("FRAGMENT", "Float64 predicate examples, bare calls"),

    # Common mistakes section — intentionally wrong code
    1359: ("FRAGMENT", "Wrong: missing contracts"),
    1366: ("FRAGMENT", "Wrong: missing effects clause"),
    1379: ("FRAGMENT", "Wrong: missing effects clause (with contracts)"),
    1389: ("FRAGMENT", "Wrong: bare expression without indices"),
    1426: ("FRAGMENT", "Wrong: missing index on slot reference"),
    1431: ("FRAGMENT", "Correct: expression with indices (not full fn)"),
    1438: ("FRAGMENT", "Wrong: match arm with incorrect return"),
    1508: ("FRAGMENT", "Wrong: non-exhaustive match (missing None)"),
    1521: ("FRAGMENT", "Wrong: non-exhaustive match (missing arm)"),

    # Import syntax — intentionally unsupported
    1554: ("FRAGMENT", "Wrong: import aliasing not supported"),
    1559: ("FRAGMENT", "Correct: import syntax example"),
    1569: ("FRAGMENT", "Wrong: import hiding not supported"),
    1574: ("FRAGMENT", "Correct: multi-import syntax"),

    # Match arm fragment — bare match body
    1483: ("FRAGMENT", "Match arm bare expression"),
    1528: ("FRAGMENT", "Correct: match expression example (bare)"),
    1543: ("FRAGMENT", "Correct: if/else with braces (common mistakes)"),

    # String escapes — bare expression
    1588: ("FRAGMENT", "Correct escape sequence examples, bare strings"),

    # Map/Set common mistakes — bare let bindings
    1596: ("FRAGMENT", "Wrong: standalone map_new/set_new without type context"),
    1602: ("FRAGMENT", "Correct: map_new/set_new with type context"),

    # Effect disambiguation — qualified calls
    1153: ("FRAGMENT", "Qualified effect calls (State.put, Logger.put)"),
    ...
    122: ("MISMATCH", "Function signature template with @ParamType placeholders"),
}
