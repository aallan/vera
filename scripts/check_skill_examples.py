...
ALLOWLIST: dict[int, tuple[str, str]] = {
    ...
    1355: ("FRAGMENT", "Effect handler syntax template"),
    ...
    863: ("FRAGMENT", "HTML built-in examples, bare calls"),
    879: ("FRAGMENT", "HTML match expression example, bare expression"),
    ...
    827: ("FRAGMENT", "Markdown built-in examples, bare calls"),

    # Regex operations — bare function calls
    894: ("FRAGMENT", "Regex built-in examples, bare calls"),
    905: ("FRAGMENT", "Regex Result matching example, bare expression"),
    ...
    917: ("FRAGMENT", "Numeric built-in examples, bare calls"),

    # Math built-ins (#467) — bare function calls (log/trig/pi/clamp)
    932: ("FRAGMENT", "Math built-in examples: log/trig/constants/clamp, bare calls"),

    # Contracts section — requires/ensures fragments
    996: ("FRAGMENT", "Requires clause example, not full function"),
    1005: ("FRAGMENT", "Ensures clause example, not full function"),

    # Quantified expressions — bare forall/exists calls
    1055: ("FRAGMENT", "Quantified expression examples, bare calls"),
    ...
    1338: ("FRAGMENT", "Handler syntax template, not real code"),

    # Effect declarations — bare effects(...) clauses
    1073: ("FRAGMENT", "Effect declarations list"),
    1182: ("FRAGMENT", "Async effect declarations list"),
    1232: ("FRAGMENT", "Http effect declarations list"),
    1263: ("FRAGMENT", "Inference effect declarations list"),

    # Qualified calls and handler fragments — bare expressions
    1271: ("FRAGMENT", "Handler with clause, bare expression"),

    # Line comments — bare comments
    1571: ("FRAGMENT", "Comment syntax example"),

    # Type conversions — bare function calls
    954: ("FRAGMENT", "Type conversions, bare calls"),

    # Common mistakes section — intentionally wrong code
    1650: ("FRAGMENT", "Wrong: missing contracts"),
    1670: ("FRAGMENT", "Wrong: missing effects clause (with contracts)"),
    1717: ("FRAGMENT", "Wrong: missing index on slot reference"),
    1722: ("FRAGMENT", "Correct: expression with indices (not full fn)"),
    1834: ("FRAGMENT", "Wrong: non-exhaustive match (missing None)"),
    1812: ("FRAGMENT", "Wrong: non-exhaustive match (missing arm)"),

    # Import syntax — intentionally unsupported
    1845: ("FRAGMENT", "Wrong: import aliasing not supported"),
    1850: ("FRAGMENT", "Correct: import syntax example"),
    1860: ("FRAGMENT", "Wrong: import hiding not supported"),

    # Match arm fragment — bare match body
    1819: ("FRAGMENT", "Correct: match expression example (bare)"),
    1829: ("FRAGMENT", "Common mistake example, bare if/else"),
    1799: ("FRAGMENT", "Correct: if/else with braces (common mistakes)"),

    # String escapes — bare expression
    1879: ("FRAGMENT", "Correct escape sequence examples, bare strings"),

    # Map/Set common mistakes — bare let bindings
    1887: ("FRAGMENT", "Wrong: standalone map_new/set_new without type context"),
    1893: ("FRAGMENT", "Correct: map_new/set_new with type context"),

    # Effect disambiguation — qualified calls
    1377: ("FRAGMENT", "Qualified effect calls (State.put, Logger.put)"),
    ...
    1202: ("FRAGMENT", "Async effect row declarations, bare clauses"),
    1367: ("FRAGMENT", "Handler with-clause, bare put arm expression"),

    # Float64 predicates — bare function calls (shifted by De Bruijn section additions)
    967: ("FRAGMENT", "Float64 predicate and constant examples, bare calls"),

    # Contracts incremental workflow — placeholder scaffolding, not a full function
    1035: ("FRAGMENT", "Contracts scaffolding template: requires(true) ensures(true)"),

    # String utilities and character classification — bare function-call signatures
    791: ("FRAGMENT", "String utility / classifier signatures, bare function calls"),
