...
The model doesn't need to be right. It needs to be *checkable*. Names are replaced by structural references. Contracts are mandatory. Effects are typed. Every function is a specification the compiler checks against its implementation: proving what it can, guarding most of the rest at run time, and saying which it can do neither for, with known gaps where a contract is recorded as guarded but is not ([#1607](https://github.com/aallan/vera/issues/1607), [#1530](https://github.com/aallan/vera/issues/1530), [#1582](https://github.com/aallan/vera/issues/1582)).
...
- **Language server** — A warm Z3 session between keystrokes — proofs re-check at editor latency. Custom methods hand agents [proof deltas]({RAW}/LSP_SERVER.md) before an edit lands.
...
Live proof-aware diagnostics, hover, slot go-to-definition, and typed-hole completion come from the [language server]({RAW}/LSP_SERVER.md). The source install above (`.[dev]`) already includes it; on the PyPI route add it with `python -m pip install "veralang[lsp]"`, or use `pip install -e ".[lsp]"` for a lighter source checkout. Any editor with a generic LSP client can point at `vera lsp` directly.
...
- [`TOOLCHAIN.md`]({RAW}/TOOLCHAIN.md) — The CLI cookbook: driving the toolchain to write, verify, test, run, and debug Vera, plus the `builtins`/`effects`/`errors` introspection commands.
...
For other models: point them at [`SKILL.md`]({SITE}/SKILL.md) via system prompt, file attachment, or retrieval. It's self-contained and works with any model that reads markdown. Every Vera example in it, and on this page, is checked, verified and, where it names an expected output, run in CI, or carries a marker that says why it is not.
