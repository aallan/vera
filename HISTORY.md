...
| v0.0.134 | 6 May | **Active reclamation of host-store handles (heap-wrap-as-ADT) — closes [#573](https://github.com/aallan/vera/issues/573), [#575](https://github.com/aallan/vera/issues/575), [#576](https://github.com/aallan/vera/issues/576)**.  All three host-handle types (Map, Set, Decimal) now lower to GC-tracked wrapper-ADT pointers; Phase 2c of `$gc_collect` fires `host_decref_handle` to evict unreachable wrappers from the corresponding Python-side store.  A 10 000-iter `map_insert` chain leaves `_map_store` at 1 entry, not 10 001.  Closes the entire host-handle leak class in one PR. |
