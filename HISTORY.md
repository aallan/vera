...
*The first agent-written Conway's Life ran cleanly end-to-end — and the path there exposed a fresh batch of friction.*

Stage 12 opens on the morning v0.0.138 shipped: the residual GC-rooting bug in #593 closed, `life.vera` running 200+ generations of Gosper Glider Gun + R-pentomino + Pentadecathlon at 80×22 with zero corruption, and a green wall on the test suite.  The campaign that ran from v0.0.120 closed sixteen runtime/codegen bugs.  But the day started with 81 open issues and ended with 94 — the agent experiments that drove the bug-killing surfaced their own residual: scale-only bugs that small focused tests miss ([#596](https://github.com/aallan/vera/issues/596) stress harness), walker-completeness gaps with the same shape as #588 ([#597](https://github.com/aallan/vera/issues/597) walker audit), browser-runtime gaps that turn `--target browser` from "works" into "approximate" ([#609](https://github.com/aallan/vera/issues/609) JSPI sleep + [#610](https://github.com/aallan/vera/issues/610) ANSI subset interpreter), and codegen bugs revealed only by real programs ([#602](https://github.com/aallan/vera/issues/602) String interp WASM + [#604](https://github.com/aallan/vera/issues/604) prelude combinators silently skipped).  Stage 12 is the close-out: build the infrastructure that prevents the recurrence, finish the browser runtime, then resume the agent-integration push.
...
| v0.0.141 | 8 May | Inline-refinement return types reproduce the same #602 i64/i32 mismatch — third trigger in the bug class.  v0.0.140's fix handled `NamedType` only; `RefinementType` (the `@{ @String \| predicate }` form) fell through.  Extracted `_resolve_i32_pair_ret_te` helper that handles both, applied to all three i32_pair branches (non-generic, generic-mono, and the parallel IndexExpr-of-FnCall element-type inference). |
| v0.0.142 | 8 May | Closed the #602 bug class structurally — [#630](https://github.com/aallan/vera/issues/630).  After ten distinct triggers across PRs #627 + #629 (each fixed locally with one more `isinstance` handler or one more inference site, with the 9th and 10th landing within hours of #630 being filed), the discovery rate had outpaced reactive fixing.  Tier 1 consolidated six overlapping canonicalisation helpers in `vera/wasm/inference.py` into a single `_canonical_named_type` walker (RefinementType unwrap + alias-chain follow + generic substitution + type_args formatting) plus a `_canonical_wasm_type` convenience wrapper.  Migrated all callers to delegate; deleted the duplicate `_resolve_type_name_to_wasm_canonical`; collapsed apply_fn dispatchers from 75-line nested isinstance ladders to 18 lines.  Tier 2 added [E615] "Cannot interpolate value of unknown type" and converted the silent `to_string(...)` fallthrough in `_translate_interpolated_string` (the load-bearing amplifier behind every #602-class miscompilation) to a loud compile-time skip. |
...
| Tests | ~50 | ~300 | ~600 | ~1,400 | ~2,300 | 3,095 | 3,716 |
...
Total: **810+ commits, 141 tagged releases, 56 active development days.**
