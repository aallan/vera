...
| v0.0.121 | 27 Apr | **Nested closures + ADT capture work end-to-end** ([#514](https://github.com/aallan/vera/issues/514), [#527](https://github.com/aallan/vera/issues/527)) — second from the bug-killing campaign. Closure-lifting becomes a worklist that bubbles inner closures up; sigs and IDs share by-reference with module-level state to avoid cross-context name collisions. Closures inside closure bodies (the natural 2D `array_map` shape) compile and run at any depth and any return type. `_walk_free_vars` now recurses into nested `AnonFn` so captures resolve correctly. Pair-type captures (`String`, `Array<T>`) split into the residual [#535](https://github.com/aallan/vera/issues/535). Plus drop the CVE-2026-3219 ignore — pip 26.1 shipped the fix. |
...
Total: **810+ commits, 120 tagged releases, 40 active development days.**
