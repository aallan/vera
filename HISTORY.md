...
| — | 9 Apr | **Bug fix: nested `handle[State<T>]` isolation** — each handler now pushes a fresh state cell on entry and pops it on exit, so nested handlers of the same type have independent state; inner handler mutations no longer corrupt the outer handler's value (#417). Added `state_push_T`/`state_pop_T` host imports (Python + browser/Node runtimes), updated `_is_void_expr` to handle `HandleExpr`, and added three `TestEffectHandlers` tests covering nested State<Int> isolation. |
