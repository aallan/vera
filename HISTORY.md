...
| v0.0.159 | 28 May | **`Map<K, T_heap>` and `Set<T_heap>` no longer drop heap-pointer values under GC pressure** ([#695](https://github.com/aallan/vera/issues/695) + [#705](https://github.com/aallan/vera/issues/705)).  Pre-fix: `_map_store[handle][key] = heap_ptr` and `_set_store[handle] = {heap_ptr, ...}` held heap pointers as Python ints, invisible to the conservative GC scan; a `$gc_collect` between map / set construction and value access reclaimed those blocks.  Symptom on `VERA_EAGER_GC=1`: `map_get` returning pointers to freed memory, `set_to_array` producing dead JArray pointers — silent UAF, no trap.  Fix mirrors the host-store contents into a WASM-resident bucket array at wrapper body offset +8, so the conservative scan reaches values via shadow stack → wrapper → bucket → val_ptr.  Three populating paths covered: the WAT-emitted `attach_bucket_to_wrapper` host import (CLI + browser parallel), the host-side `_alloc_map_wrapper` (JSON / HTML parser path), and match-arm / let-binding shadow-rooting for heap-pointer bindings (the orthogonal #695 root cause).  Decimal exempt — value-typed.  Mirror approach; architectural "move" cleanup tracked as [#706](https://github.com/aallan/vera/issues/706). |
