...
| — | 17 Apr | **Widen GC object header size field from 16-bit to 31-bit** ([#484](https://github.com/aallan/vera/issues/484)) — the sweeper was masking size readback with `0xFFFF`, silently truncating allocations ≥65536 bytes and shredding payload via spurious free-list links. Discovered during #480 PR 1 when the 10K-element `array_map` stress test corrupted its input array. Masks removed at all five read sites in `vera/codegen/assembly.py`. Single allocations now bounded by WASM's 4 GB memory ceiling. |
| — | 17 Apr | **Iterative WASM higher-order array ops** ([#480](https://github.com/aallan/vera/issues/480)) — `array_map`, `array_filter`, and `array_fold` all migrated from recursive prelude Vera functions to iterative WAT loops with O(1) shadow-stack depth. Shipped across three PRs ([#485](https://github.com/aallan/vera/pull/485), [#486](https://github.com/aallan/vera/pull/486), [#489](https://github.com/aallan/vera/pull/489)). Fold's pair/ADT accumulator case needed an in-place shadow-stack slot overwrite to keep the running pointer rooted across closure-invocation GC cycles without per-iteration push/pop. Establishes the iterative-combinator pattern for future `array_sort`/`array_reverse`/`array_mapi` in [#466](https://github.com/aallan/vera/issues/466). |
| v0.0.114 | 17 Apr | **`IO.sleep`, `IO.time`, `IO.stderr`** ([#463](https://github.com/aallan/vera/issues/463)) — three new operations on the built-in `IO` effect. `IO.sleep(@Nat)` pauses for N milliseconds, `IO.time(@Unit)` returns current Unix time in ms, `IO.stderr(@String)` writes to stderr separate from stdout. Enables animation loops, elapsed-time measurement, and stderr-separated CLI tools. Python runtime uses `time.sleep`/`time.time`/`sys.stderr`; browser runtime busy-waits on `performance.now()` (no `Atomics.wait` on the main thread), uses `Date.now()`, and captures stderr to a separate buffer accessible via `getStderr()`. Discovered missing while writing a Conway's Game of Life program. |
| v0.0.115 | 18 Apr | **`Random` effect** ([#465](https://github.com/aallan/vera/issues/465)) — new built-in effect with three operations: `random_int(@Int, @Int)` (inclusive range), `random_float(@Unit)` (uniform `[0.0, 1.0)`), `random_bool(@Unit)` (coin flip). Functions declaring `effects(<Random>)` surface non-determinism in their type signatures. Python runtime backs onto the stdlib `random` module; browser runtime backs all three onto `Math.random()`. No seeding / `handle[Random]` yet — that's future work. Unblocks games, simulations, Monte Carlo methods, shuffling, and randomized initial states. |
| v0.0.116 | 20 Apr | **Math built-ins** ([#467](https://github.com/aallan/vera/issues/467)) — fifteen new pure functions across four groups: logarithmic (`log`, `log2`, `log10`), trigonometric (`sin`, `cos`, `tan`, `asin`, `acos`, `atan`, `atan2`), constants (`pi()`, `e()`), and numeric utilities (`sign`, `clamp`, `float_clamp`). Log/trig route through host imports wrapping Python's `math` / browser's `Math`; constants inline as WAT `f64.const`; `sign` inlines as a branchless `(x > 0) - (x < 0)` comparison sequence; `clamp` inlines as two WASM `select` instructions evaluating `min(max(v, lo), hi)`; `float_clamp` uses native `f64.max` / `f64.min`. Gated per-op so unused ones don't bloat the module. Unlocks scientific computing, graphics, audio, physics, and statistics workloads. |
| — | 22 Apr | **Dependabot uv ecosystem + auto-uv-lock workflow** ([#500](https://github.com/aallan/vera/pull/500), [#501](https://github.com/aallan/vera/pull/501)) — switched `dependabot.yml` from the `pip` ecosystem to `uv` so generated PRs include `uv.lock` regeneration alongside `pyproject.toml` bumps, and added a belt-and-braces GitHub Actions workflow that runs `uv lock` on any dependabot-authored PR touching `pyproject.toml` and pushes the regenerated lockfile back to the branch. Resolves a recurring CI lint failure where dependabot bumps left `uv.lock` out of sync with the manifest. Subsequent dependency wave (mypy 1.20.2, ruff 0.15.11, pre-commit 4.6.0, wasmtime 44.0.0) merged cleanly. |
| v0.0.117 | 22 Apr | **Array utility built-ins (phase 1)** ([#466](https://github.com/aallan/vera/issues/466)) — seven new array combinators completing the higher-order operations suite: `array_mapi` (map with index — collapses the recursive-accumulator-with-index pattern that was a leading source of De Bruijn indexing mistakes), `array_reverse`, `array_find` (returns `Option<T>`), `array_any` / `array_all` (short-circuiting universal/existential quantifiers with vacuous-truth on empty input), `array_flatten` (one-level concatenation via two-pass length-then-copy), and `array_sort_by` (stable insertion sort with caller-supplied `Ordering`-returning comparator). All seven are iterative WASM with O(1) shadow-stack depth, mirroring the [#480](https://github.com/aallan/vera/issues/480) `array_map` / `array_filter` / `array_fold` pattern. Phase 2 (`array_sort` / `array_contains` / `array_index_of` with `Ord`/`Eq` ability dispatch) tracked separately — that work needs `compare$T` / `eq$T` reified as first-class function handles. Discovered along the way: niladic ADTs (`Less`/`Equal`/`Greater`) are heap-allocated boxes with the tag at offset 0, not raw i32 tags; the comparator return must be dereferenced before tag comparison. |
| v0.0.118 | 23 Apr | **String utilities + character classification** ([#470](https://github.com/aallan/vera/issues/470), [#471](https://github.com/aallan/vera/issues/471)) — sixteen new built-ins shipped as a single PR, all inline WAT (no host imports, bit-identical between `wasmtime` and Node.js). String utilities: `string_chars` (canonical bridge from `String` to `Array<String>`; each 1-byte element is independently `$alloc`-ed to keep the GC mark phase honest about interior-pointer rooting), `string_lines` (Python `splitlines()` — `\n` / `\r\n` / `\r` terminators; trailing terminator does not add empty segment), `string_words` (Python `split()` — whitespace runs collapse, empty segments discarded), `string_reverse`, `string_trim_start`, `string_trim_end`, `string_pad_start`, `string_pad_end` (JavaScript `padStart`/`padEnd` — fill cycles left-to-right and is truncated to exactly the padding length; empty fill is a no-op). Character classification: `is_digit`, `is_alpha`, `is_alphanumeric`, `is_whitespace`, `is_upper`, `is_lower` (first-byte ASCII predicates; `false` for empty string). First-character case conversion: `char_to_upper`, `char_to_lower` (only the first byte is transformed; remaining bytes pass through — useful for title-casing). ASCII-only; Unicode-aware variants tracked separately. Eliminates the hand-rolled `byte == 48 or byte == 49 or ...` patterns and gives agents the string primitives that every non-trivial parser needs. |
| v0.0.119 | 23 Apr | **JSON typed accessors** ([#366](https://github.com/aallan/vera/issues/366)) — eleven new accessor functions shipped as **pure-Vera prelude functions**, not WASM translators. Six Layer-1 type-coercion accessors (`json_as_string`, `json_as_number`, `json_as_bool`, `json_as_int`, `json_as_array`, `json_as_object`) return `Option<T>` from a `Json` value, `Some` when the constructor matches and `None` otherwise. Five Layer-2 compound field accessors (`json_get_string`, `json_get_number`, `json_get_bool`, `json_get_int`, `json_get_array`) collapse `json_get(obj, key)` and the matching `json_as_*` coercion into a single call — the pattern 90% of real API-consuming code wants. `json_as_int` guards every `float_to_int` trap path (NaN, ±infinity, and finite overflow when |f| >= 2^63), returning `None` for every non-representable-as-Int input. Eliminates the two-level pattern-match boilerplate every JSON API consumer would otherwise write. First non-WASM-translator stdlib addition of Stage 11 — proves the prelude-function path scales for ergonomic combinators that don't need inline WAT. |
...
Total: **810+ commits, 119 tagged releases, 40 active development days.**
