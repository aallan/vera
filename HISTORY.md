...
| Version | Date | What shipped |
|---------|------|-------------|
| — | 7 May | `examples/life.vera` — Conway's Game of Life: nested array combinators, recursive `<IO>` loop, ANSI rendering, formal B3/S23 rule on `next_cell`. |
...
| v0.0.159 | 28 May | **`Map<K, T_heap>` and `Set<T_heap>` no longer drop heap-pointer values under GC pressure, on both CLI and browser targets** ([#695](https://github.com/aallan/vera/issues/695) + [#705](https://github.com/aallan/vera/issues/705) + [#708](https://github.com/aallan/vera/issues/708)).  Pre-fix: `_map_store[handle][key] = heap_ptr` and `_set_store[handle] = {heap_ptr, ...}` held heap pointers as Python ints, invisible to the conservative GC scan; the same shape applied to the browser-runtime `writeJson` / `writeHtml` walkers, which allocated trees of heap blocks without rooting intermediates.  Symptom on `VERA_EAGER_GC=1`: `map_get` returning pointers to freed memory, `set_to_array` producing dead JArray pointers, `json_array_length` reading freed JArray bodies — silent UAF, no trap.  Fix mirrors the host-store contents into a WASM-resident bucket array at wrapper body offset +8 (CLI + browser), adds match-arm / let-binding shadow-rooting for heap-pointer bindings (the orthogonal #695 root cause), and ports the #692 host-walker `_ShadowGuard` discipline from v0.0.158 to JS-side via new `gcGuard` / `gcShadowPush` / `gcShadowPop` helpers (the missing fix the browser regression tests surfaced).  Decimal exempt — value-typed.  Mirror approach; architectural "move" cleanup tracked as [#706](https://github.com/aallan/vera/issues/706). |
