...
| v0.0.119 | 23 Apr | **JSON typed accessors** ([#366](https://github.com/aallan/vera/issues/366)) — eleven new accessor functions shipped as **pure-Vera prelude functions**, not WASM translators. Six Layer-1 type-coercion accessors (`json_as_string`, `json_as_number`, `json_as_bool`, `json_as_int`, `json_as_array`, `json_as_object`) return `Option<T>` from a `Json` value, `Some` when the constructor matches and `None` otherwise. Five Layer-2 compound field accessors (`json_get_string`, `json_get_number`, `json_get_bool`, `json_get_int`, `json_get_array`) collapse `json_get(obj, key)` and the matching `json_as_*` coercion into a single call — the pattern 90% of real API-consuming code wants. `json_as_int` guards the `float_to_int` trap on NaN and infinity, returning `None` for those inputs. Eliminates the two-level pattern-match boilerplate every JSON API consumer would otherwise write. First non-WASM-translator stdlib addition of Stage 11 — proves the prelude-function path scales for ergonomic combinators that don't need inline WAT. |
