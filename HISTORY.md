...
| v0.0.138 | 7 May | **Closure-return shadow-push asymmetry — closes [#593](https://github.com/aallan/vera/issues/593)** (the residual Conway's Life string corruption from gen 1+ at 12×30 grid).  `_compile_lifted_closure` only emitted the return-value `gc_shadow_push` when the closure body itself allocated, but `array_map` / `array_mapi` always emit the matching pop after each `call_indirect` when the element type is heap-pointer-like.  Non-allocating closures returning Strings (like `fn(@Bool -> @String) { render_cell(@Bool.0) }`) had no push but the loop popped anyway, dropping `$gc_sp` below the surrounding function's prologue baseline and corrupting earlier roots.  Diagnosed via the new **`VERA_EAGER_GC=1`** debug knob (forces `$gc_collect` on every alloc — see [`ENVIRONMENT.md`](ENVIRONMENT.md)) which surfaced the imbalance on the very first iteration rather than only at scale.  Fix: lift the return-value push out of the `if ctx.needs_alloc:` branch so it always runs when the return is a heap pointer.  `life_full_program.vera` at 12×30 × 200 generations now runs cleanly with zero U+FFFD corruption. |
