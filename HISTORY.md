...
| v0.0.144 | 11 May | Tier A bug burn-down — closes [#633](https://github.com/aallan/vera/issues/633) (defensive cycle guard on `_resolve_base_type_name` with `_root_name` thread for prefix-chain cycles), [#634](https://github.com/aallan/vera/issues/634) (`SlotRef` source spans inside `InterpolatedString` remapped from synthetic-wrapper coordinates to original-source via `_remap_spans_inplace`; `[E615]` diagnostics now point at the exact segment line + column), [#556](https://github.com/aallan/vera/issues/556) (regression-pinned the pure-Unit-helper-in-statement-position shape that #584 closed in v0.0.135), [#591](https://github.com/aallan/vera/issues/591) (HTTP/Inference UTF-8 hygiene — `Http.get`/`Http.post` decode response bodies with `errors="replace"`, `Inference.complete` catches `UnicodeDecodeError` and re-raises as a Vera-shaped message).  Also: `_split_interpolation` escape-skipping aligned with `_has_interpolation` (CodeRabbit-flagged pre-existing bug — `"\\("` was mis-segmented as an interpolation opener).  Filed [#648](https://github.com/aallan/vera/issues/648) (cyclic-alias `RecursionError` — the user-visible upstream bug discovered while validating #633's reachability). |
